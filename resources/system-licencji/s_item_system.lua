...
function useItem(itemSlot, additional)
	...
	if itemID then
		if (itemID==1) then -- hotdog
			...
			exports.global:sendLocalMeAction(source, "zjadł hot doga.")
		...
		elseif (itemID==4) or (itemID==5) then -- house key or business key
			...
			if found then
				local dbid, entrance, exit, interiorType, interiorElement = exports['system-domow']:findProperty( source, found )
				...
				exports['antyczit']:changeProtectedElementDataEx(interiorElement, "status", interiorStatus, true)	
			else
		...
		elseif (itemID==8) then -- sandwich
			...
			exports.global:sendLocalMeAction(source, "zjadł kanapke")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==9) then -- sprunk
			...
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			exports.global:sendLocalMeAction(source, "wypija sprunka.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==10) then -- red dice
			exports.global:sendLocalText(source, " *((Kostka)) " .. getPlayerName(source):gsub("_", " ") .. " toczy sie i wskazuje liczbę " .. math.random( 1, 6 ) ..".", 255, 51, 102)
		elseif (itemID==11) then -- taco
			giveHealth(source, 10)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "zjadł taco")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==12) then -- cheeseburger
			giveHealth(source, 10)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			setTimer(removeAnimation, 4000, 1, source)
			exports.global:sendLocalMeAction(source, "zjadł cheeseburger.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==13) then -- donut
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "zjadł pączka")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==14) then -- cookie
			giveHealth(source, 80)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "zjadł ciasteczko.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==15) then -- water
			giveHealth(source, 90)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "wypija butelkę wody.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==16) then -- clothes
			...
			if fittingskins[gender] and fittingskins[gender][race] and fittingskins[gender][race][skin] then
				...
				if exports['antyczit']:changeProtectedElementDataEx(source, "casualskin", skin, false) then
					mysql:query_free("UPDATE characters SET casualskin = " .. skin .. " WHERE id = " .. getElementData(source, "dbid") )
				end
				exports.global:sendLocalMeAction(source, "przebrał się.")
				if vehicle then
		...
		elseif (itemID==17) then -- watch
			exports.global:sendLocalMeAction(source, "spogląda na zegarek.")
			...
			if (getElementData(source, "duty") > 0) and (getElementModel(source) == 285) then
				...
				if not (found) then
					...
					elseif (interiorStatus[3]) then
						...
						exports['antyczit']:changeProtectedElementDataEx(found, "status", interiorStatus, true)
		...
		elseif (itemID>=34 and itemID<=44) then -- DRUGS
			...
			if getPedOccupiedVehicle(source) and ( itemID == 38 or itemID == 42 ) then
				outputChatBox("Użyłeś troszke " .. itemName .. ", ale nic sie nie dzieje...", source, 255, 0, 0)
			else
				exports.global:sendLocalMeAction(source, "zażył " .. itemName .. ".")
			end
		...
		elseif (itemID==50) then -- highway code book
			...
			exports.global:sendLocalMeAction(source, "czyta ".. bookTitle ..".")
		...
		elseif (itemID==54) then -- GHETTOBLASTER
			exports.global:sendLocalMeAction(source, "kładzie radio na ziemie.")
			local x, y, z = unpack(additional)
			
			triggerEvent("dropItem", source, itemSlot, x, y, z+0.3)
		elseif (itemID==55) then -- Praow jazdy
			exports.global:sendLocalMeAction(source, "zagląda do kieszeni po czym wyciąga prawo jazdy.")
			outputChatBox("Prawo jazdy, umożliwia ono poruszanie sie na drodze pojazdem.", source, 255, 51, 102)
		elseif (itemID==57) then -- FUEL CAN
			...
			if found then
				...
				outputChatBox("Jesteś za daleko auta.", source, 255, 194, 14)
			end
		elseif (itemID==58) then
			takeItemFromSlot(source, itemSlot)
			exports.global:sendLocalMeAction(source, "wypił piwo.")
			setElementHealth(source,getElementHealth(source)-5)
		elseif (itemID==59) then -- MUDKIP
			takeItemFromSlot(source, itemSlot)
			exports.global:sendLocalMeAction(source, "zjadł..... mudkip.")
			--killPed(source)
		elseif (itemID==60) then
			local x,y,z = getElementPosition(source)
			local rz = getPedRotation(source)
			local dimension = getElementDimension(source)
			local retval = call(getResourceFromName("system-domow"), "addSafeAtPosition", source, x,y,z, rz) --0 no error, 1 safe already exists, 2 player does not own interior
			if (retval == 0) then
		...
		elseif (itemID==62) then
			takeItemFromSlot(source, itemSlot)
			exports.global:sendLocalMeAction(source, " wypija całą butelke vodki.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==63) then
			takeItemFromSlot(source, itemSlot)
			exports.global:sendLocalMeAction(source, "wypija całą szkocką.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==69) then -- Dictionary
			local learned = call(getResourceFromName("system-jezykow"), "learnLanguage", source, itemValue, true)
			local lang = call(getResourceFromName("system-jezykow"), "getLanguageName", itemValue)
		...
		elseif (itemID==72) then -- Note
			exports.global:sendLocalMeAction(source, "odczytuje notatke.")
		...
		elseif (itemID==77) then -- Card Deck
			...
			exports.global:sendLocalText(source, " *((Karty)) " .. getPlayerName(source):gsub("_", " ") .. " draws a card and gets a" .. ( number == 1 and "n" or "" ) .. " " .. cards[number] .." of ".. sign[snumber] ..".", 255, 51, 102)
		...
		elseif (itemID==83) then -- Coffee
			...
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "wypija kawę.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==89) then -- Generic Food
			...
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "zjadł " .. itemName .. ".")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==92) then
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "zjadł kurczaczka.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==93) then
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "zjadł świąteczny budyń.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==94) then
			...
			exports.global:sendLocalMeAction(source, "otwiera prezent")
		elseif (itemID==95) then -- Generic Drink
			giveHealth(source, tonumber(getItemValue(itemID, itemValue)))
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "wypija " .. itemName .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==96) then -- PDA
			exports.global:sendLocalMeAction(source, "włącza swoje " .. ( itemValue == 1 and "PDA" or itemValue ) .. " .")
		...
		elseif (itemID==99) then --[ADDED: 2/22/10 by herbjr] tray
			...
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			exports.global:sendLocalMeAction(source, "spożywa jedzenie z tacy.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==100) then --[ADDED: 2/22/10 by herbjr] milk
			...
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			exports.global:sendLocalMeAction(source, "wypija mały kartkonik mleka.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==101) then --[ADDED: 2/22/10 by herbjr] juice
			...
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			exports.global:sendLocalMeAction(source, "wypija mały kartownik z soczkiem.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==102) then --[ADDED: 2/23/10 by herbjr] Cabbage (Asked for by Misha)
			...
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
			exports.global:sendLocalMeAction(source, "zjada głowę kapusty.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==104) then -- portable TV
			...
			if isTvUsed == nil or isTvUsed == false then
				...
				exports['antyczit']:changeProtectedElementDataEx(source, "isTvUsed", true, false)
			else
				exports['antyczit']:changeProtectedElementDataEx(source, "isTvUsed", false, false)
			end
		elseif (itemID == 105) then -- Pack of cigarettes. Withdraw one if it still has one
			if (itemValue > 0) then
				if (hasSpaceForItem(source, 106, 1)) then
					...
					exports.global:sendLocalMeAction(source, "zagląda do paczki papierosów i wyciąga jednego.")
				else
					outputChatBox( "Twój inwentarz jest pełny.", source, 255, 0, 0 )
				end
			else
				exports.global:sendLocalMeAction(source, "zagląda do paczki papierosów lecz ona jest pusta.")
			end
		elseif (itemID == 106) then -- Cigarette
			if hasItem( source, 107 ) then
				exports.global:sendLocalMeAction(source, "zapala papierosa.")
				outputChatBox( "(( uzyj komendy /wyrzuc by wyrzucic papierosa ,przekładać papierosa możesz komendą /zmienreke ))", source )
				triggerEvent("realism:startsmoking", source, 0) -- 0 = left hand -- 1 = right hand
				takeItemFromSlot(source, itemSlot)
			else
				exports.global:sendLocalMeAction(source, "Pokazuje wszystkim papierosa.")
				outputChatBox( "You'll need a lighter to use it properly, tho.", source, 255, 0, 0 )
			end
		elseif (itemID == 107) then
			exports.global:sendLocalMeAction(source, "pokazuje że ma zapalniczke.")
		elseif (itemID==108) then -- donut
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "spożywa naleśnik.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==109) or (itemID==110) then -- Fruit
			giveHealth(source, tonumber(getItemValue(itemID, itemValue)))
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "zjada " .. itemName .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID == 113) then -- Pack of glowsticks, Withdraw one if it still has one
			if (itemValue > 0) then
				if (hasSpaceForItem(source, 28, 1)) then
					...
					exports.global:sendLocalMeAction(source, "zagląda wewnątrz paczki paluszków żarowych, wyciągając jeden.")
				else
					outputChatBox( "Twój inwentarz jest pełny.", source, 255, 0, 0 )
				end
			else
		...
		elseif badges[itemID] then
			if ( getElementData( source, badges[itemID][1] ) ) then
				exports['antyczit']:changeProtectedElementDataEx(source, badges[itemID][1], false, true)
				exports.global:sendLocalMeAction(source, "ściągnął " .. badges[itemID][2] .. ".")
			else
				for key, badge in pairs ( badges ) do
					if key ~= itemID then
						if ( getElementData ( source, badge[1] ) ) then
							exports['antyczit']:changeProtectedElementDataEx( source, badge[1], false, true )
							exports.global:sendLocalMeAction( source, "ściągął " .. badge[2] .. "." )
						end
					end
				end

				exports['antyczit']:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(), true )
				exports.global:sendLocalMeAction( source, "zakłada  " .. badges[itemID][2] .. "." )
			end
		...
		elseif masks[itemID] then
			...
			if mask then
				...
				exports['antyczit']:changeProtectedElementDataEx(source, data[1], false, true)
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				...
				exports['antyczit']:changeProtectedElementDataEx(source, data[1], true, true)
			end
		else
...
function destroyItem(itemID)
	...
	outputChatBox("Skasowałeś przedmiot " .. itemName .. ".", source, 255, 194, 14)
	exports.global:sendLocalText(source, " ** " .. getPlayerName(source):gsub("_", " ") .. " zniszczył " .. itemName .. ".", 255, 51, 102) 
end
...
function showItem(itemName)
	if isPedDead(source) or getElementData(source, "injuriedanimation") then return end
	exports.global:sendLocalMeAction(source, "pokazuje wszystkim " .. itemName .. ".")
end
