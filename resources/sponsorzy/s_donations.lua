		...
		if (logged == true) then
			...
			if (gameAccountID) then
				if (gameAccountID > 0) then
					...
					local perksTable = { }
					exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "donation-system:perks", perksTable, false)
					if (mysqlResult) then
						while true do
	...
	if (hasPlayerPerk(targetPlayer, perkID)) then
		...
		if (gameAccountID) then
			if (gameAccountID > 0) then
				...
				exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "donation-system:perks", perkTable, false)
				return true
			end
		end
	end
		...
		if (gameAccountID > 0) then
			...
			if (tonumber(perkID) == 15) then -- Add a vehicle slot, max 10
				if (characterID and tonumber(characterID) > 0) then
					...
					exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "maxvehicles", currentMaxVehicles)
					...
					setElementData(targetPlayer, "account:credits",tonumber(getElementData(targetPlayer,"account:credits") or 0)-points)
					loadAllPerks(targetPlayer)
					outputChatBox("dodatek aktywny: Zwiększona ilośc aut do " .. currentMaxVehicles .. ".", targetPlayer)
					return true, "Success"
				end
			elseif (tonumber(perkID) == 18) then
				if (characterID and tonumber(characterID) > 0) then
					...
					if exports.global:giveItem(targetPlayer, 2, number) then
						...
						outputChatBox("Dodatek aktywny: twój nowy numer to " .. number .. ".", targetPlayer)
						return true, "Success"
					else
				...
				outputChatBox("dodatek Aktywowany", targetPlayer)
				return true, "Success"
			end
		end
	end
...
function onCharacterSpawn(characterName, factionID)
	...
	if (togPMperk) then
		exports['antyczit']:changeProtectedElementDataEx(source, "pmblocked", tonumber(togPMstatus), false)
	end
				
	local togADperk, togADstatus = hasPlayerPerk(source, 2)
	if (togADperk) then
		exports['antyczit']:changeProtectedElementDataEx(source, "disableAds", tonumber(togadminsADstatus) == 1, false)
	end
			
	local togNewsPerk, togNewsStatus = hasPlayerPerk(source, 3)
	if (togNewsPerk) then
		exports['antyczit']:changeProtectedElementDataEx(source, "tognews", tonumber(togNewsStatus), false)
	end
