...
function openDonationGUI(obtained, available, credits)
	...
	wDonation = guiCreateWindow(0.20, 0.20, 0.6, 0.6, "Dodatki", true)
	guiWindowSetSizable(wDonation, false)
		
	lSpendText = guiCreateLabel(0.015, 0.05, 0.5, 0.15, "Nieprzydzielone punkty: " .. tostring(credits) .." Punktów", true, wDonation)
	guiSetFont(lSpendText, "default-bold-small")
	
	bClose = guiCreateButton(0.5, 0.05, 0.5, 0.075, "Zamknij", true, wDonation)
	addEventHandler("onClientGUIClick", bClose, closeDonationGUI)
	
	lActive = guiCreateLabel(0.015, 0.15, 0.15, 0.15, "Aktywne Produkty:", true, wDonation)
	...
	if (#obtained == 0) then
		lItems[9999] = guiCreateLabel(0.015, y+0.0015, 0.4, 0.1, "- Brak", true, wDonation)
		guiSetFont(lItems[9999], "default-bold-small")
	end
	
	lAvailable = guiCreateLabel(0.40, 0.15, 0.15, 0.15, "Dostępne dodatki:", true, wDonation)
	guiSetFont(lAvailable, "default-bold-small")
	local y = 0.2
	for perkID, perkArr in ipairs(available) do
		if (perkArr[1] ~= nil) and (perkArr[2] ~= 0) then
			lItems[perkID] = guiCreateLabel(0.40, y+0.0015, 0.44, 0.1, "- "..perkArr[1] .. "\n   ".. ( perkArr[3] > 1 and (perkArr[3] .." dni - ") or "") .. perkArr[2] .." Punktów", true, wDonation)
			guiSetFont(lItems[perkID], "default-bold-small")
			
			if credits >= perkArr[2] then
				bItems[perkID] = guiCreateButton(0.85, y+0.01, 0.1, 0.03, "Aktywuj", true, wDonation)
				...
				guiSetText(lItems[perkID], guiGetText(lItems[perkID]) .. " - Posiadasz zbyt mało punktów")
			end
			y = y + 0.06
		end
	end
	...
	if (#available == 0) then
		lItems[9998] = guiCreateLabel(0.30, y+0.0015, 0.4, 0.1, "- Brak", true, wDonation)
...
function showPhonePicker()
	...
	wPhone = guiCreateWindow(x, y, width, height, "Dodatki", false)

	guiCreateLabel(0.03, 0.20, 2.0, 0.1, "Wpisz swój unikalny numer:", true, wPhone)
	...
	lNumber = guiCreateLabel(0.03, 0.41, 2.0, 0.16, "Wpisz poprawny numer (cyfry)", true, wPhone)
	guiLabelSetColor(lNumber, 255, 0, 0)
	
	--Buttons
	local cancel = guiCreateButton(0.15, 0.75, 0.70, 0.10, "Zmień", true, wPhone)
	addEventHandler("onClientGUIClick", cancel, hidePhonePicker, false)
	bNumber = guiCreateButton(0.15, 0.60, 0.70, 0.10, "Ok!", true, wPhone)
...
function checkNumber()
	...
	if valid then
		guiSetText(lNumber, "Numer zajęty, kliknij 'Ok' by\n zobaczyć dostępne numery.")
