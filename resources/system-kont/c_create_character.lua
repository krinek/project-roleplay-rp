...
function newCharacter_init()
	...
	gui["_root"] = guiCreateWindow(0, screenY/2-225, 255, 557, "Kreacja Postaci", false)
	guiWindowSetSizable(gui["_root"], false)
		
	gui["lblCharName"] = guiCreateLabel(10, 25, 91, 16, "Dane osobowe", false, gui["_root"])
	...
	gui["lblCharNameExplanation"] = guiCreateLabel(10, 45, 240, 80,"Nazwa musi mieć następujący format: \n     Imie oraz Nazwisko\nPrzykład: Joe Harisson. Musisz uzywać\nrealnych imion i nazwisk.", false, gui["_root"])
	...
	gui["lblCharDesc"] = guiCreateLabel(10, 125, 120, 16, "Opis postaci:", false, gui["_root"])
	...
	gui["lblCharDescExplanation"] = guiCreateLabel(10, 245, 231, 61, "Wpisz opis swojej postaci  \nprzykładem może być miejsce przybycia lub opis osobowości .", false, gui["_root"])
	...
	gui["lblGender"] = guiCreateLabel(10, 343, 46, 13, "Płeć", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblGender"], "left", false)
	guiLabelSetVerticalAlign(gui["lblGender"], "center")
	gui["rbMale"] = guiCreateRadioButton(90, 342, 51, 17, "Mezczyzna", false, gui["_root"])
	gui["rbFemale"] = guiCreateRadioButton(150, 342, 82, 17, "Kobieta", false, gui["_root"])
	...
	gui["lblWeight"] = guiCreateLabel(10, 426, 111, 16, "Waga", false, gui["_root"])
	...
	gui["lblAge"] = guiCreateLabel(10, 447, 111, 16, "Wiek", false, gui["_root"])
	...
	gui["lblLanguage"] = guiCreateLabel(10, 468, 111, 16, "Ojczysty język", false, gui["_root"])
	...
	gui["lblLanguageDisplay"] = guiCreateLabel(130, 468, 111, 16, "Polski", false, gui["_root"])
	...
	gui["btnCancel"] = guiCreateButton(10, 495, 231, 21, "Anuluj", false, gui["_root"])
	addEventHandler("onClientGUIClick", gui["btnCancel"], newCharacter_cancel, false)
	
	gui["btnCreateChar"] = guiCreateButton(10, 515, 231, 41, "Swtórz postać!", false, gui["_root"])
...
function newCharacters_getSkinArray()
	...
	if (guiCheckBoxGetSelected( gui["chkBlack"] )) then -- BLACK
		...
		elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then -- BLACK FEMALE
			...
			outputChatBox("Wybierz Płeć", 0, 255, 0)
		end
...
function newCharacter_attemptCreateCharacter()
	...
	if not nameCheckPassed then
		LoginScreen_showWarningMessage( "Błąd, twoje dane osobowe:\n".. nameCheckError )
		return
	end
	
	local characterDescription = guiGetText(gui["memCharDesc"])
	if #characterDescription < 50 then
		LoginScreen_showWarningMessage( "Błąd, twój \nopis: Jest zbyt krótki." )
		return
	elseif #characterDescription > 128 then
		LoginScreen_showWarningMessage( "Popełniłeś błędy w opisie/danych\nJest zbyt długa." )
		return
	end
	...
	elseif (guiCheckBoxGetSelected(gui["chkAsian"])) then
		...
		LoginScreen_showWarningMessage( "Błąd w kolorze skóry:\nNie wybrałeś." )
		return
	end
	...
	elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then
		...
		LoginScreen_showWarningMessage( "Błąd w wybieraniu płci twojej postaci:\nNie wybrałeś jej." )
		return
	end
	
	local skin = getElementModel( dummyPed )
	if not skin then
		LoginScreen_showWarningMessage( "Błąd w wybieraniu skina  :\nSkin nie wybrany." )
		return
	end
	...
	if languageselected == nil then
		LoginScreen_showWarningMessage( "Błąd w wyborze języka:\nNie wybrałeś go." )
		return
	end
...
function newCharacter_response(statusID, statusSubID)
	if (statusID == 1) then
		LoginScreen_showWarningMessage( "Oops, coś poszło nie tak. Spróbuj ponownien\nlub skontaktuj sie z supportem.\nError ACC"..tostring(statusSubID) )
	elseif (statusID == 2) then
		if (statusSubID == 1) then
			LoginScreen_showWarningMessage( "Te dane osobe są juz w użyciu n\nuse, wybacz :(!" )
		else
			LoginScreen_showWarningMessage( "Oops, coś poszło nie tak \nskontaktuj sie z supportem.\nError ACD"..tostring(statusSubID) )
		end
...
fileDelete("c_create_character.lua")