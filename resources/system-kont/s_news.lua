    ...
    callRemote( newsURL,
		function( title, text, author, date )
			if title == "ERROR" then
				...
				exports['antyczit']:changeProtectedElementDataEx( resourceRoot, "news:title", title )
				exports['antyczit']:changeProtectedElementDataEx( resourceRoot, "news:text", text )
				exports['antyczit']:changeProtectedElementDataEx( resourceRoot, "news:sub", author .. " on " .. date )
				if player then
