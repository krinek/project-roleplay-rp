...
addEventHandler("realism:startsmoking", getRootElement(),
	function(hand)
		...
		exports['antyczit']:changeProtectedElementDataEx(source, "realism:smoking", true, false )
		exports['antyczit']:changeProtectedElementDataEx(source, "realism:smoking:hand", hand, false )
...
function stopSmoking(thePlayer)
	...
	if (isElement(thePlayer)) then	
		...
		if (isSmoking) then
			...
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "realism:smoking", false, false )
		end
	end
end
...
function stopSmokingCMD(thePlayer)
	...
	if (isSmoking) then
		...
		exports.global:sendLocalMeAction(thePlayer, "wyrzucił papierosa na ziemię.")
	end
end
addCommandHandler("wyrzuc", stopSmokingCMD)
...
function changeSmokehand(thePlayer)
	...
	if (isSmoking) then
		...
		exports['antyczit']:changeProtectedElementDataEx(thePlayer, "realism:smoking:hand",1-smokingHand, false )
	end
end
addCommandHandler("zmienreke", changeSmokehand)
