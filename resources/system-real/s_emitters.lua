...
function createEmitter(thePlayer, commandName, type)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (type) or (type - 0 > 3) or (type - 0 < 1) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Typ żywiołu]", thePlayer, 255, 194, 14)
			outputChatBox("Typ 1: Mały pożar", thePlayer, 255, 194, 14)
			outputChatBox("Typ 2: Duży pożar", thePlayer, 255, 194, 14)
			outputChatBox("Typ 3: Strumien wody (jak z hydrantu) ", thePlayer, 255, 194, 14)
		else
			local id = #emitters + 1
			...
			outputChatBox("Żywioł utworzony z ID " .. id .. " i typem " .. type .. ".", thePlayer, 0, 255, 0)
		end
	end
end
addCommandHandler("zywiol", createEmitter, false, false)

function nearbyEmitters(thePlayer)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		local count = 0
		outputChatBox("Żywioły: ", thePlayer, 255, 194, 15)
		...
		for key, value in ipairs(emitters) do
			...
			if ( getDistanceBetweenPoints3D(x, y, z, px, py, pz) < 50) and getElementDimension( thePlayer ) == getElementDimension( value[5] ) then
				count = count + 1
				outputChatBox("Żywioł z ID " .. key .. " i typem " .. type .. " utworzony przez "..creator..".", thePlayer, 255, 194, 15)
			end
		end
...
addCommandHandler("zywioly", nearbyEmitters)

function delEmitter(thePlayer, commandName, id)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (id) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Emitter ID]", thePlayer, 255, 194, 14)
		else
			if ( emitters[tonumber(id)] == nil ) then
				...
				destroyElement(obj)
				outputChatBox("Żywioł z ID " .. id .. " został usunięty.", thePlayer, 0, 255, 0)
			end
		end
	end
end
addCommandHandler("usunzywiol", delEmitter)
...
function delEmitters(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		emitters = {}
		outputChatBox("Usunięto " .. count .. " Żywioły.", thePlayer, 0, 255, 0)
	end
end
addCommandHandler("usunzywioly", delEmitters)