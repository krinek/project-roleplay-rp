	...
	if c_lastspeed - c_speed >= 0.25 and not isElementFrozen( c_veh ) then
		if (c_lastspeed - c_speed >= 0.35) then -- trigger throwing out of the vehicle
			...
			if (bcollision) then
				...
				setTimer(triggerServerEvent,250,1,"crashThrowPlayerFromVehicle", vehicle, ex, ey, ez+1, vehicle)
			else
				setTimer(triggerServerEvent,250,1,"crashThrowPlayerFromVehicle", vehicle, nx, ny, nz+1, vehicle)
			end
		end
...
fileDelete("c_vehicle_crash.lua")