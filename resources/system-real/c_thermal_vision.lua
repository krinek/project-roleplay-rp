	...
	for key, value in ipairs(getElementsByType("player")) do
		if isElementStreamedIn(value) and (isElementOnScreen(value)) and (localPlayer~=value) and tV ~= getPedOccupiedVehicle(value) then
			...
			if (isLineOfSightClear(px, py, pz, x, y, z, true, false, false, true, false, false, true, tV)) then
				...
				-- needs fixing
				if (blackMales[skin]) then text = "Czarnoskóry"
				elseif (whiteMales[skin]) then text = "Rasa Biała"
				elseif (asianMales[skin]) then text = "Rasa Azjatycka"
				elseif (blackFemales[skin]) then text = "Czarna Kobieta"
				elseif (whiteFemales[skin]) then text = "Biała Kobieta"
				elseif (asianFemales[skin]) then text = "Azjatka"
				else text = "Rasa Biała"
				end
...
fileDelete("c_thermal_vision.lua")