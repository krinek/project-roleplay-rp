...
function applyCruiseControl()
	...
	if (theVehicle) then
		if (getVehicleOccupant(theVehicle) == getLocalPlayer()) then
			if (ccEnabled) then
				outputChatBox("Tempomat Wyłączony", 255,0,0)
				deactivateCruiseControl()
			else
				...
				if targetSpeed > 4 then
					if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) == "Bike" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train") then
						outputChatBox("Tempomat Włączony", 0,255,0)
						outputChatBox("Użyj - i + by zmieniać ustawienia prędkości", 0,255,0)
...
function bindMe()
    ...
    function loadMe( startedRes )
		outputDebugString("Tempomat wczytany")
		...
		addCommandHandler("tempo", applyCruiseControl)
		addCommandHandler("tempomat", applyCruiseControl)
...
fileDelete("c_cruise_control.lua")