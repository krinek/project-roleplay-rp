...
function payWage(player, pay, faction, tax)
	...
	if (exports.sponsorzy:hasPlayerPerk(player, 4)) then
		donatormoney = donatormoney + 25
	end
	
	if (exports.sponsorzy:hasPlayerPerk(player, 5)) then
	...
	exports['antyczit']:changeProtectedElementDataEx(player, "businessprofit", 0, false)
	local bankmoneyadd = math.max( 0, pay ) + interest + profit + donatormoney
	local bankmoneyorig = bankmoney
	bankmoney = bankmoney + bankmoneyadd-- + math.max( 0, pay ) + interest + profit + donatormoney
	...
	if (rent > 0) then
		if (rent > bankmoney)   then
			rent = -1
			call( getResourceFromName( "system-domow" ), "publicSellProperty", player, rented, false, true )
		else
	...
	-- save the bankmoney
	exports['antyczit']:changeProtectedElementDataEx(player, "bankmoney", bankmoney-(bankmoneyadd-interest), true)

	mysql:query_free( "UPDATE characters SET wyplataMinutyKasa = wyplataMinutyKasa + "..bankmoneyadd-interest.." WHERE id = " .. dbid )

	-- let the client tell them the (bad) news
	local grossincome = pay+profit+interest+donatormoney-rent-vtax-ptax
	triggerClientEvent(player, "cwyplata", player, faction, noWage and -1 or pay, profit, interest, donatormoney, tax, incomeTax, vtax, ptax, rent, grossincome)
...
function payAllWages(timer)
	if timer then
		...
		--]]
		setTimer(payAllWages, 60000*6, 1, true)
	end
	loadWelfare( )
	threads = { }
	taxVehicles = {}
	...
	for _, value in ipairs(players) do
		local co = coroutine.create(dowyplataPlayer)
...
function dowyplataPlayer(value, hourly)
	if hourly then
	coroutine.yield()
	end
	--exports.global:sendMessageToAdmins("wyplata: Processing thread for " .. getPlayerName(value))
	local sqlupdate = ""
	local logged = getElementData(value, "loggedin")
	local timeinserver = getElementData(value, "timeinserver")
	local dbid = getElementData( value, "dbid" )
	local dutyLevel = getElementData( value, "duty")
	
	if dutyLevel and dutyLevel ~= 0 and timeinserver >= 5 then
	if (logged==1) and (getPlayerIdleTime(value) < 600000) then
		local result = mysql:query_fetch_assoc("SELECT wyplataMinuty FROM characters WHERE id=" .. dbid )
		mysql:query_free( "UPDATE characters SET wyplataMinuty = wyplataMinuty + 6 WHERE id = " .. dbid )
		--local result = mysql:query_fetch_assoc("SELECT wyplataMinuty FROM characters WHERE id=" .. dbid .. " AND `active` = 1")
		local wyplataMinuty = tonumber(result.wyplataMinuty)
		if type(wyplataMinuty) ~="number" then return end
		wyplataMinuty = wyplataMinuty + 6
		if wyplataMinuty % 60 == 0 then
			...
			if carLicense and carLicense < 0 then
				exports['antyczit']:changeProtectedElementDataEx(value, "license.car", carLicense + 1, false)
			...
			if gunLicense and gunLicense < 0 then
				exports['antyczit']:changeProtectedElementDataEx(value, "license.gun", gunLicense + 1, false)
			...
			exports['antyczit']:changeProtectedElementDataEx(value, "timeinserver", math.max(0, timeinserver-6), false, true)
			local hoursplayed = getElementData(value, "hoursplayed") or 0
			exports['antyczit']:changeProtectedElementDataEx(value, "hoursplayed", hoursplayed+1, false, true)
	...
	elseif (getPlayerIdleTime(value) > 600000) then
		--exports.global:sendMessageToAdmins("[wyplata] No wyplata for '"..getPlayerName(value):gsub("_", " ").."' due more as 10 minutes no movement.")
	elseif (logged==1) and (timeinserver) and (timeinserver<60) then
		--outputChatBox("Grasz obecnie zbyt krótko by otrzymać wypłate. (You require another " .. 60-timeinserver .. " Minutes of play.)", value, 255, 0, 0)
	end
	end
end

function adminDowyplata(thePlayer)
...
addCommandHandler("dowyplata", adminDowyplata)

function adminDowyplataOne(thePlayer, commandName, targetPlayerName)
	local logged = getElementData(thePlayer, "loggedin")
...
addCommandHandler("wyplata", adminDowyplataOne)
...
function timeSaved(thePlayer)
	...
	if (logged==1) then
		...
		outputChatBox("Obecnie grasz na serwerze " .. timeinserver .. " minut.", thePlayer, 255, 195, 14)
		--outputChatBox("Wymagane jest grać minimun " .. 60-timeinserver .. " minut by odebrać pierwszą wypłatę.", thePlayer, 255, 195, 14)
	end
end
...
local bot_marker = createMarker( 359.0537109375, 166.0009765625, 1008.3828125- 0.8, "cylinder", 1, 255, 255, 0, 128 )
local bot_ped = createPed(150,356.0400390625, 165.765625, 1008.3768310547)
...
addEventHandler( "onMarkerHit", bot_marker,
	function( thePlayer, dimension )
		...
		local result = mysql:query_fetch_assoc("SELECT wyplataMinuty,wyplataMinutyKasa FROM characters WHERE account='" .. mysql:escape_string(tostring(clientAccountID)) .. "' AND `active` = 1")
		if result and result.wyplataMinuty and result.wyplataMinutyKasa then
			if tonumber(result.wyplataMinuty) and tonumber(result.wyplataMinutyKasa) then
				if tonumber(result.wyplataMinuty) < 60 then
					exports.info:showBox (thePlayer,"info","Nie przepracowałeś wystarczająco dużo czasu, aby odebrać wypłatę.")
				else
					local kasa = tonumber(result.wyplataMinutyKasa)
					local mins = tonumber(result.wyplataMinuty)
					local hours = (mins-(mins%60))/60
					exports.global:giveMoney( thePlayer, kasa )
					mysql:query_free("UPDATE characters SET wyplataMinutyKasa=0,wyplataMinuty=wyplataMinuty-".. hours*60 .." WHERE account='" .. mysql:escape_string(tostring(clientAccountID)) .. "' AND `active` = 1")
					exports.info:showBox (thePlayer,"info","Otrzymaleś $"..kasa.." za przepracowane "..hours.."h.")
				end
			end
		end
