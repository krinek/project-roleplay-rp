...
addEventHandler( "onClientResourceStart", getResourceRootElement( getThisResource() ), 
	function ( resource )
		...
		bindKey( triggerKey, "both", toggleScoreboard )
...
function toggleScoreboard( _, state )
	state = iif( state == "down", true, false )
	if state then
					...
					for key, player in ipairs( players ) do
						...
						for key, column in ipairs( scoreboardColumns ) do
							...
							elseif column.name == "id" then
								content = getElementData (player,"playerid")
							elseif column.name == "ooc" then
								content = getElementData (player,"account:username")
							elseif column.name == "gp" then
								content = getElementData (player,"account:credits")
							else
								content = getElementData( player, column.name )
							end
							content = iif( content and column.name ~= "name" and type( content ) ~= "table", tostring( content ), content )
...
--			if serverInfo.server and showServerInfo then
			...
			dxDrawText( "http://project-roleplay.pl/", topX+s(5), y+s(1), topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
...
--[[
			...
			while ( index < firstVisibleIndex+maxPerWindow and scoreboardContent[index] ) do
				...
				else--]]if element and isElement( element ) and getElementType( element ) == "player" then
