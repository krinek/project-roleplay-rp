...
local mainRes = { "system-administracji", "system-kont", "global", "mysql", "pool" }
	...
	for i, res in ipairs(mainRes) do
		...
		if (resName) then
			...
			if (cState ~= "running") then
				displayAdminM("UWAGA '" .. res .. "' nie jest włączony , należy go niezwłocznie włączyć.")
...
function restartSingleResource(thePlayer, commandName, resourceName)
	if (getElementData(thePlayer, "adminlevel") >= 5) then
		if not (resourceName) then
			outputChatBox("PRZYKŁAD: /resrestart [Resource Name]", thePlayer, 255, 194, 14)
		else
...
function startSingleResource(thePlayer, commandName, resourceName)
	if (getElementData(thePlayer, "adminlevel") >= 5) then
		if not (resourceName) then
			outputChatBox("PRZYKŁAD: /resstart [Resource Name]", thePlayer, 255, 194, 14)
		else
			local theResource = getResourceFromName(tostring(resourceName))
			if (theResource) then
				if getResourceState(theResource) == "running" then
