...
local function checkStreamIn()
	if ( enabled ) then
		...
		for key, value in pairs(getElementsByType(element, getResourceDynamicElementRoot(getResourceFromName ( "system-przedmiotow")))) do
...
function applyPickupClientConfigSettings()
	local streamerEnabled = tonumber( exports['system-kont']:loadSavedData("streamer-"..element.."-enabled", "1") )
	...
	local streamerDistance = tonumber( exports['system-kont']:loadSavedData("streamer-"..element, "25") )
