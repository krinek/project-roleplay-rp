...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, memberLocation, factionRanks, factionWages, factionTheTeam, note)
	if (gFactionWindow==nil) then
		...
		tabOverview = guiCreateTab("Ogólne", tabs)
		
		-- Make members list
		gMemberGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabOverview)
		
		colName = guiGridListAddColumn(gMemberGrid, "Dane", 0.23)
		colRank = guiGridListAddColumn(gMemberGrid, "Ranga", 0.20)
		colOnline = guiGridListAddColumn(gMemberGrid, "Status", 0.115)
		colLastLogin = guiGridListAddColumn(gMemberGrid, "Ostatnio online", 0.17)
		...
		if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) then
			--colLocation = guiGridListAddColumn(gMemberGrid, "Lokacja", 0.25)
			colWage = guiGridListAddColumn(gMemberGrid, "Pensja ($)", 0.1)
		else
			--colLocation = guiGridListAddColumn(gMemberGrid, "Lokacja", 0.35)
		end
		...
		for k, v in ipairs(memberUsernames) do
			...
			if (not memberLastLogin[k]) then
				...
				elseif (memberLastLogin[k]==1) then
					login = tostring(memberLastLogin[k]) .. " dzień temu"
				else
					login = tostring(memberLastLogin[k]) .. " dni temu"
				end
			end
			...
			if (memberLeaders[k]) then
				...
				guiGridListSetItemText(gMemberGrid, row, colLeader, "Pracownik", false, false)
			end
		...
		if (localPlayerIsLeader) then
			gButtonKick = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Zwolnij gracza", true, tabOverview)
			gButtonLeader = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Włacz status lidera", true, tabOverview)
			gButtonPromote = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Awansuj Gracza", true, tabOverview)
			gButtonDemote = guiCreateButton(0.825, 0.3058, 0.16, 0.06, "Zniż range gracza", true, tabOverview)
			
			if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) then
				gButtonEditRanks = guiCreateButton(0.825, 0.3824, 0.16, 0.06, "Edytuj rangi i pensje", true, tabOverview)
			else
				gButtonEditRanks = guiCreateButton(0.825, 0.3824, 0.16, 0.06, "Edytuj rangi", true, tabOverview)
			end
			
			gButtonEditMOTD = guiCreateButton(0.825, 0.459, 0.16, 0.06, "Edytuj Opis Frakcji", true, tabOverview)
			gButtonInvite = guiCreateButton(0.825, 0.5356, 0.16, 0.06, "Zatrudnij Gracza", true, tabOverview)
			gButtonRespawn = guiCreateButton(0.825, 0.6122, 0.16, 0.06, "Zrespawnuj Pojazdy", true, tabOverview)
			
			local factionID = getElementData(getLocalPlayer(), "faction")
			local factionPackages = exports.dyzur:getFactionPackages(factionID)
			--outputDebugString(tostring(factionPackages))
			if factionPackages and #factionPackages > 0 then
				gButtonPerk = guiCreateButton(0.825, 0.6878, 0.16, 0.06, "Zarządzaj dyżurem", true, tabOverview)
			...
			gButtonQuit = guiCreateButton(0.825, 0.7834, 0.16, 0.06, "Zwolnij sie", true, tabOverview)
			gButtonExit = guiCreateButton(0.825, 0.86, 0.16, 0.06, "Zamknij Okno", true, tabOverview)
...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		for i=1, 20 do
			y = ( y or 0 ) + 23
			lRanks[i] = guiCreateLabel(0.05 * width, y + 3, 0.4 * width, 20, "Ranga #" .. i .. " Ranga i Pensja: ", false, wRanks)
		...
		bRanksSave = guiCreateButton(0.05, 0.900, 0.9, 0.045, "Zapisz", true, wRanks)
		bRanksClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Zamknij", true, wRanks)
...
function btEditMOTD(button, state)
	if (source==gButtonEditMOTD) and (button=="left") and (state=="up") then
		if not (wMOTD) then
			...
			wMOTD = guiCreateWindow(x, y, width, height, "Wiadomość dnia", false)
			tMOTD = guiCreateEdit(0.1, 0.2, 0.85, 0.1, tostring(theMotd), true, wMOTD)
			
			guiSetInputEnabled(true)
			
			bUpdate = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Aktualizuj", true, wMOTD)
			addEventHandler("onClientGUIClick", bUpdate, sendMOTD, false)
			
			bMOTDClose= guiCreateButton(0.1, 0.775, 0.85, 0.15, "Zamknij okno", true, wMOTD)
...
function btInvitePlayer(button, state)
	if (source==gButtonInvite) and (button=="left") and (state=="up") then
		if not (wInvite) then
			...
			wInvite = guiCreateWindow(x, y, width, height, "Zatrudnij Gracza", false)
			tInvite = guiCreateEdit(0.1, 0.2, 0.85, 0.1, "Dane osobowe gracza", true, wInvite)
			addEventHandler("onClientGUIChanged", tInvite, checkNameExists)
					
			lNameCheck = guiCreateLabel(0.1, 0.325, 0.8, 0.3, "znaleziono gracza.", true, wInvite)
			...
			bInvite = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Zatrudnij!", true, wInvite)
			guiSetEnabled(bInvite, false)
			addEventHandler("onClientGUIClick", bInvite, sendInvite, false)
			
			bCloseInvite = guiCreateButton(0.1, 0.775, 0.85, 0.15, "Zamknij okno", true, wInvite)
...
function checkNameExists(theEditBox)
	...
	if (count>1) then
		...
		guiSetEnabled(bInvite, false)
	elseif (count==1) then
		guiSetText(lNameCheck, "znaleziono gracza. ("..foundstr..")")
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		if (numLeaders==1) and (isLeader) then
			...
			wConfirmQuit = guiCreateWindow(sx/2 - 125,sy/2 - 50,250,100,"Decyzja", false)
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Jesteś pewien opuszczenia  " .. getTeamName(theTeam) .. "?",true,wConfirmQuit)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
			local bYes = guiCreateButton(0.1,0.65,0.37,0.23,"Tak",true,wConfirmQuit)
			local bNo = guiCreateButton(0.53,0.65,0.37,0.23,"Nie",true,wConfirmQuit)
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		elseif (playerName~="") then
			...
			outputChatBox(" " .. playerName .. " z frakcji '" .. tostring(theTeamName) .. "'Zwolniony.", 0, 255, 0)
			triggerServerEvent("cguiKickPlayer", getLocalPlayer(), playerName)
		else
			outputChatBox("Wybierz gracza do zwolnienia.")
		end
	end
end
...
function btButtonPerk(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPerk) then
		...
		wPerkWindow = guiCreateWindow(x, y, width, height, "Profity frakcji "..playerName, false)
		...
		bPerkSave = guiCreateButton(0.05, 0.900, 0.9, 0.045, "Zapisz", true, wPerkWindow)
		bPerkClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Zamknij", true, wPerkWindow)
	...
	if (button=="left") and (state=="up") and (source==gButtonLeader) then
		...
		elseif (playerName~="") then
			...
			if (currentLevel=="Leader") then
				guiGridListSetItemText(gMemberGrid, row, tonumber(colLeader), "Pracownik", false, false)
			...
			outputChatBox("Wzkaz gracza do ustawienia liderem.")
		end
	end
end
	...
	if (button=="left") and (state=="up") and (source==gButtonPromote) then
		...
		if (playerName~="") then
			...
			if (currRankNumber==20) then
				outputChatBox(playerName .. " posiada juz najwyższa range.", 255, 0, 0)
			else
				guiGridListSetSelectedItem(gMemberGrid, 0, 0)
	...
	if (button=="left") and (state=="up") and (source==gButtonDemote) then
		...
		if (playerName~="") then
			...
			if (currRankNumber==1) then
				outputChatBox(playerName .. " posiada już najniższą range.", 255, 0, 0)
			else
				guiGridListSetSelectedItem(gMemberGrid, 0, 0)
				guiGridListSetItemText(gMemberGrid, row, colRank, arrFactionRanks[currRankNumber-1], false, false)
				if colWage then
...
fileDelete("c_faction_system.lua")