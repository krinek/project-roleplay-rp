...
function loadAllFactions(res)
	...
	if result then
		while true do
			...
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "type", factionType, true)
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "money", money, false)
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "id", id, true)
			...
			local motd = row.motd
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "ranks", factionRanks, false)
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "wages", factionWages, false)
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "motd", motd, false)
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "note", row.note == mysql_null() and "" or row.note, false)
		...
		for k, thePlayer in ipairs(players) do
			...
			if result then
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "factionMenu", 0, false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "faction", tonumber(result.faction_id), false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "factionrank", tonumber(result.faction_rank), false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "factionleader", tonumber(result.faction_leader), false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "factionPackages", fromJSON(result.faction_perks) or { }, false)
		...
		if (menuVisible==0) then
			...
			if (factionID~=-1) then
				...
				if query then
					...
					exports['antyczit']:changeProtectedElementDataEx(source, "factionMenu", 1, false)
				...
				outputChatBox("Nie jesteś w żadnej frakcji.", source)
			end
		else
...
function callbackUpdateRanks(ranks, wages)
	...
	if (wages) then
		for i = 1, 20 do
			wages[i] = math.min(85000, math.max(0, tonumber(wages[i]) or 0))
		end
		...
		exports['antyczit']:changeProtectedElementDataEx(theTeam, "wages", wages, false)
	end
	...
	exports['antyczit']:changeProtectedElementDataEx(theTeam, "ranks", ranks, false)
	...
	if not (factionCooldown) then
		for key, value in ipairs(exports.pool:getPoolElementsByType("vehicle")) do
			...
			if (faction == factionID and not getVehicleOccupant(value, 0) and not getVehicleOccupant(value, 1) and not getVehicleOccupant(value, 2) and not getVehicleOccupant(value, 3) and not getVehicleTowingVehicle(value)) then
				...
				exports['antyczit']:changeProtectedElementDataEx(value, "handbrake", 1, false)
		...
		for k, v in ipairs(teamPlayers) do
			outputChatBox(username .. " zrespawnował wszystkie nieużywane pojazdy.", v)
		end

		setTimer(resetFactionCooldown, 600000, 1, theTeam)
		exports['antyczit']:changeProtectedElementDataEx(theTeam, "cooldown", true, false)
	else
		outputChatBox("Respawn aut był dokonany przed chwilą ..", source, 255, 0, 0)
	end
end
...
function resetFactionCooldown(theTeam)
	exports['antyczit']:changeProtectedElementDataEx(theTeam, "cooldown")
end
...
function callbackUpdateNote(note)
	...
	if (factionID~=-1) then
		if mysql:query_free("UPDATE factions SET note='" .. tostring(mysql:escape_string(note)) .. "' WHERE id='" .. factionID .. "'") then
			...
			exports['antyczit']:changeProtectedElementDataEx(theTeam, "note", note, false)
		else
...
function callbackRemovePlayer(removedPlayerName)
	...
	if mysql:query_free("UPDATE characters SET faction_id='-1', faction_leader='0', faction_rank='1', dutyskin = 0, duty = 0 WHERE charactername='" .. mysql:escape_string(removedPlayerName) .. "'") then
		...
		if (removedPlayer) then -- Player is online
			...
			outputChatBox(username .. " zwolnił cie z frakcji '" .. tostring(theTeamName) .. "'", removedPlayer)
			setPlayerTeam(removedPlayer, getTeamFromName("Citizen"))
			exports['antyczit']:changeProtectedElementDataEx(removedPlayer, "faction", -1, false)
			exports['antyczit']:changeProtectedElementDataEx(removedPlayer, "dutyskin", -1, false)
			exports['antyczit']:changeProtectedElementDataEx(removedPlayer, "factionleader", 0, false)
...
function callbackPerkEdit( perkIDTable, playerName)
	...
	if mysql:query_free("UPDATE `characters` SET `faction_perks`='" .. mysql:escape_string(jsonPerkIDTable) .. "' WHERE `charactername`='" .. mysql:escape_string(playerName) .. "'") then
		outputChatBox(" Dyżur został zaaktualizowany dla "..playerName:gsub("_", " ")..".", client, 255, 0, 0)
		local targetPlayer = getPlayerFromName(playerName)
		if targetPlayer then
			setElementData(targetPlayer, "factionPackages", perkIDTable)
			outputChatBox(" Twój dyżur został zmieniony przez "..getPlayerName(client):gsub("_", " ") .. ".", targetPlayer, 255, 0, 0)
		end
	end
end
...
function callbackToggleLeader(playerName, isLeader)
	...
	if (isLeader) then -- Make player a leader
		...
		if mysql:query_free("UPDATE characters SET faction_leader='1' WHERE charactername='" .. mysql:escape_string(playerName) .. "'") then
			...
			local teamPlayers = getPlayersInTeam(theTeam)
			for k, v in ipairs(teamPlayers) do
				outputChatBox(username .. " awansował " .. playerName .. " na lidera.", v)
			end
			
			local thePlayer = getPlayerFromName(playerName)
			if(thePlayer) then -- Player is online, tell them
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "factionleader", 1, true)
			end
		else
		...
		if mysql:query_free("UPDATE characters SET faction_leader='0' WHERE charactername='" .. mysql:escape_string(playerName) .. "'") then
			...
			if(thePlayer) then -- Player is online, tell them
				...
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "factionleader", 0, true)
			end
...
function callbackPromotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if mysql:query_free("UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. mysql:escape_string(playerName) .. "'") then
		...
		if(thePlayer) then -- Player is online, set his rank
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "factionrank", rankNum, false)
		end
		...
		if (theTeam) then
			local teamPlayers = getPlayersInTeam(theTeam)
			for k, v in ipairs(teamPlayers) do
				outputChatBox(username .. " awansuje " .. playerName .. " z rangi '" .. oldRank .. "' ba rangę '" .. newRank .. "'.", v)
			end
		end
	else
...
function callbackDemotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if mysql:query_free("UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. safename .. "'") then
		...
		if (theTeam) then
			local teamPlayers = getPlayersInTeam(theTeam)
			for k, v in ipairs(teamPlayers) do
				outputChatBox(username .. " zniżył range dla " .. playerName .. " z '" .. oldRank .. "' na '" .. newRank .. "'.", v)
			end
		end
	else
...
function callbackQuitFaction()
	...
	if mysql:query_free("UPDATE characters SET faction_id='-1', faction_leader='0', dutyskin = -1, duty = 0, faction_perks='{}' WHERE charactername='" .. safename .. "'") then
		outputChatBox("Zwolniłes sie z frakcji '" .. theTeamName .. "'.", client)
		
		local newTeam = getTeamFromName("Citizen")
		setPlayerTeam(client, newTeam)
		exports['antyczit']:changeProtectedElementDataEx(client, "faction", -1, false)
		exports['antyczit']:changeProtectedElementDataEx(client, "factionrank", 1, false)
		exports['antyczit']:changeProtectedElementDataEx(client, "factionleader", 0, false)
		exports['antyczit']:changeProtectedElementDataEx(client, "factionPackages", {}, false)
		--triggerClientEvent(client, "updateFactionInfo", client, -1, 1)
		exports['antyczit']:changeProtectedElementDataEx(client, "dutyskin", -1, false)
...
function callbackInvitePlayer(invitedPlayer)
	...
	if mysql:query_free("UPDATE characters SET faction_leader = 0, faction_id = " .. factionID .. ", faction_rank = 1, dutyskin = -1 WHERE charactername='" .. safename .. "'") then
		...
		if (targetTeam~=nil) and (getTeamName(targetTeam)~="Citizen") then
			...
			exports['antyczit']:changeProtectedElementDataEx(invitedPlayer, "faction", factionID, false)
			...
			if	(invitedPlayer) then
				...
				exports['antyczit']:changeProtectedElementDataEx(invitedPlayer, "factionrank", 1, false)
				--triggerClientEvent(invitedPlayer, "updateFactionInfo", invitedPlayer, factionID, 1)
				exports['antyczit']:changeProtectedElementDataEx(invitedPlayer, "dutyskin", -1, false)
...
function hideFactionMenu()
	exports['antyczit']:changeProtectedElementDataEx(client, "factionMenu", 0, false)
end
