...
addEventHandler( "onResourceStart", getResourceRootElement( ),
	function( )
		...
		while continue do
			...
			exports['antyczit']:changeProtectedElementDataEx( ped, "position", { x, y, z, rotation }, false )
	...
	function( thePlayer, commandName, type, skin, offset )
		if exports.global:isPlayerLeadAdmin( thePlayer ) then
			...
			if not type or not skin or type < 1 or type > 2 or offset < 0 or offset > 3 then
				outputChatBox( "PRZYKŁAD: /" .. commandName .. " [1=Dancer, 2=Stripper] [Skin] [Offset 0-3]", thePlayer, 255, 194, 14 )
			else
...
function updateDancing( )
	...
	for ped, options in pairs( peds ) do
		...
		local wyczytaj =(getElementData( ped, "position" )
    --outputChatBox("Wartosc wyczytaj:"..wyczytaj)
			setElementPosition( ped, wyczytaj[1], wyczytaj[2], wyczytaj[3] )
			setPedRotation( ped, wyczytaj[4] )
...
addEventHandler("onPedWasted", getResourceRootElement(),
	function()
		...
		exports['antyczit']:changeProtectedElementDataEx( newped, "dbid", getElementData( source, "dbid" ), false )
		exports['antyczit']:changeProtectedElementDataEx( newped, "position", getElementData( source, "position" ), false )
