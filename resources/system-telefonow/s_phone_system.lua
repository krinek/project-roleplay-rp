		...
		if publicphone or exports.global:hasItem(thePlayer, 2) then
			...
			if callerphoneIsTurnedOn == 0 then
				...
				if (calling) then -- Using phone already
					outputChatBox("Obecnie korzystasz z telefonu , wpisz /rozlacz.", thePlayer, 255, 0, 0)
				elseif getElementData(thePlayer, "injuriedanimation") then
					outputChatBox("Nie możesz użyć telefonu gdy jesteś nieprzytomny.", thePlayer, 255, 0, 0)
				else
					-- /me it
					if publicphone then
						exports.global:sendLocalMeAction(thePlayer, "używa publicznego telefonu.")
					else
						exports.global:sendLocalMeAction(thePlayer, "wyciąga telefon po czym dzwoni.")
					end
					
					-- If the number is a hotline aka automated machine, then..
					if isNumberAHotline(phoneNumber) then
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "phonestate", 1, false)
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "calling", phoneNumber, false)
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "callingwith", outboundPhoneNumber, false)	
					...
					-- Otherwise find a fool to answer it
						...
						-- Some basic checks.
						-- Can we afford it?
						if not exports.sponsorzy:hasPlayerPerk(thePlayer, 6) and not exports.global:hasMoney(thePlayer, 5) then
							outputChatBox("Brak stanu konta by zadzwonić.", thePlayer, 255, 0, 0)
							return
						end
						
						-- Yes, Is the target phone online or found at all?
						if not found or foundElement == thePlayer then
							outputChatBox("Poza zasięgiem.", thePlayer, 255, 194, 14)
							return
						end
						...
						if getElementData(foundElement, "calling") then
							outputChatBox("Poza zasięgiem.", thePlayer)
							return
						end
						
						-- Note down some needed details.
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "call.col", publicphone, false)
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "calling", tonumber(phoneNumber), false)
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "called", true, false)
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "callingwith", outboundPhoneNumber, false)
						exports['antyczit']:changeProtectedElementDataEx(foundElement, "calling", tonumber(outboundPhoneNumber), false)
						exports['antyczit']:changeProtectedElementDataEx(foundElement, "called", nil, false)
						...
						if not reconning then 
							...
							exports.global:sendLocalMeAction(foundElement, "'s telefon zaczyna dzwonić.")
						end
						...
						outputChatBox("Telefon dzwoni , wskazuje numer #"..tostring(phoneNumber).." '".. display .."' (( /odbierz by odebrać ))", foundElement, 255, 194, 14)
...
for i = 1, 20 do
	addCommandHandler( "call" .. tostring( i ), 
		function( thePlayer, commandName, number )
			if i <= exports['system-przedmiotow']:countItems(thePlayer, 2) then
				local count = 0
				local items = exports['system-przedmiotow']:getItems(thePlayer)
	...
	for _, phoneNumber in ipairs(phoneNumbers) do
		...
		if found and foundElement and isElement(foundElement) then
			...
			if (phoneState==0) then
				exports['antyczit']:changeProtectedElementDataEx(foundElement, "calling", nil, false)
				exports['antyczit']:changeProtectedElementDataEx(foundElement, "called", nil, false)
				exports['antyczit']:changeProtectedElementDataEx(foundElement, "call.col", nil, false)
			end
		end
	end
end
		...
		if (exports.global:hasItem(thePlayer, 2)) then
			...
			elseif (calling) then
				if (phoneState==0) then
					...
					outputChatBox("Odebrałeś telefon. (( /p by mówić ))", thePlayer)
					if not found then
						outputChatBox("You can't hear anything on the other side of the line", thePlayer)
						executeCommandHandler( "rozlacz", thePlayer )
					else
						outputChatBox("They picked up the phone.", foundElement)
						exports.global:sendLocalMeAction(thePlayer, "odbiera rozmowe.")
						exports['antyczit']:changeProtectedElementDataEx(thePlayer, "phonestate", 1, false)
						exports['antyczit']:changeProtectedElementDataEx(foundElement, "phonestate", 1, false)
						exports['antyczit']:changeProtectedElementDataEx(foundElement, "called", nil, false)
						exports.global:sendLocalMeAction(thePlayer, "wyciąga telefon z kieszeni.")
			...
			elseif not (calling) then
				outputChatBox("Twój telefon nie dzwoni.", thePlayer, 255, 0, 0)
			elseif (phoneState==1) or (phoneState==2) then
				outputChatBox("Obecnie używasz telefonu", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("Nie masz telefonu ....", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("odbierz", answerPhone)
			...
			if (calling) then
				if not (isNumberAHotline(calling)) then
					...
					if phoneState >= 1 then
						if not exports.sponsorzy:hasPlayerPerk(calling, 6) then
					...
					if found then
						if (isElement(foundElement)) then
							...
							exports['antyczit']:changeProtectedElementDataEx(foundElement, "calling", false, false)
							exports['antyczit']:changeProtectedElementDataEx(foundElement, "caller", false, false)
							exports['antyczit']:changeProtectedElementDataEx(foundElement, "call.col", false, false)
							exports['antyczit']:changeProtectedElementDataEx(foundElement, "phonestate", 0, false)
							
							local reconning2 = getElementData(foundElement, "reconx")
							if not reconning2 then
								exports.global:sendLocalMeAction(foundElement, "rozłącza sie.")
							end
				...
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "calling", false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "caller", false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "call.col", false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "phonestate", 0, false)
				
				-- Reset hotline stuff
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "callprogress", false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "call.situation", false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "call.location", false)
				
				local reconning = getElementData(thePlayer, "reconx")
				if not reconning then
					exports.global:sendLocalMeAction(thePlayer, "rozłącza sie.")
				end
...
addCommandHandler("rozlacz", hangupPhone)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then -- 2 = Cell phone item
			...
			if (phoneState==1) then
				...
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "phonestate", 2, false)
...
function talkPhone(thePlayer, commandName, ...)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then
			if not (...) then
				outputChatBox("PRZYKŁAD: /p [Message]", thePlayer, 255, 194, 14)
			elseif getElementData(thePlayer, "injuriedanimation") then
				...
				if (phoneState>=1) then 
					...
					message = call( getResourceFromName( "czat" ), "trunklateText", thePlayer, message )
					
					local callprogress = getElementData(thePlayer, "callprogress")
					if (callprogress) then
						outputChatBox("Ty [Przez telefon]: " ..message, thePlayer)
					...
					local message2 = call(getResourceFromName("system-jezykow"), "applyLanguage", thePlayer, target, call( getResourceFromName( "czat" ), "trunklateText", target, message ), language)
					outputChatBox("[" .. languagename .. "] ((" .. username .. ")) [Telefon]: " .. message2, target)
					
					-- Send the message to the person on the other end of the line
					outputChatBox("[" .. languagename .. "] You [Telefon]: " ..message, thePlayer)
					...
					if (phoneState==2) then -- Loudspeaker
						...
						for index, nearbyPlayer in ipairs(getElementsByType("player")) do
							if isElement(nearbyPlayer) and nearbyPlayer ~= target and getDistanceBetweenPoints3D(x, y, z, getElementPosition(nearbyPlayer)) < 40 and getElementDimension(nearbyPlayer) == getElementDimension(target) then
								local message2 = call(getResourceFromName("system-jezykow"), "applyLanguage", thePlayer, nearbyPlayer, call( getResourceFromName( "czat" ), "trunklateText", target, message ), language)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 7)) then
			if not (partialNick) then
				outputChatBox("PRZYKŁAD: /phonebook [Partial Name]", thePlayer, 255, 194, 14)
			else
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) then
			...
			if not number or not (...) then
				outputChatBox("PRZYKŁAD: /" .. commandName .. " [number] [message]", thePlayer, 255, 194, 14)
			elseif callerphoneIsTurnedOn == 0 then
				outputChatBox("Your phone is off.", thePlayer, 255, 0, 0)
			elseif getElementData(thePlayer, "injuriedanimation") then
				outputChatBox("You can't use your phone while knocked out.", thePlayer, 255, 0, 0)
			elseif exports.global:hasMoney(thePlayer, 1) or exports.sponsorzy:hasPlayerPerk(thePlayer, 6) then
				...
				if tonumber(number) == 921 then
					if not exports.sponsorzy:hasPlayerPerk(thePlayer, 6) then
				...
				elseif tonumber(number) == 7332 then
					...
					elseif getElementData(thePlayer, "alcohollevel") and getElementData(thePlayer, "alcohollevel") ~= 0 then
						...
						for key, value in ipairs(teamMembers) do
							...
							if(hasItem)then
								...
								message = call( getResourceFromName( "czat" ), "trunklateText", thePlayer, message )
								local message2 = call(getResourceFromName("system-jezykow"), "applyLanguage", thePlayer, value, call( getResourceFromName( "czat" ), "trunklateText", value, message ), language)
					...
					if target then
						...
						local message2 = call(getResourceFromName("system-jezykow"), "applyLanguage", thePlayer, target, message, language)
