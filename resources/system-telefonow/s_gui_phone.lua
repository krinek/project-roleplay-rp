...
function requestPhoneGUI(itemValue, newSource)
	...
	if callerphoneIsTurnedOn == 0 then
		outputChatBox("Wybierasz telefon, ale zauważ, że jest wyłączony.", client, 255,0,0)
		outputChatBox("((/togglephone lub /wlacztelefon "..ownCellnumber.." aby wlaczyc telefon))", client, 255,0,0)
		return
	end
...
function addPhoneContact(name, number, phoneBookPhone)
	if (client) then
		...
		outputChatBox("Blad, prosimy sprobowac jeszcze raz.", client, 255,0,0)
	end
end
