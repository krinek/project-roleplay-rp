...
function showPhoneGui(thePhoneNumber, contactList)
	...
	wPhoneMenu = guiCreateStaticImage(sx/2 - 150,sy/2 - 175,300,560,"iphone.png",false)
	bCall = guiCreateStaticImage(0.0733,0.0982,0.2100,0.0571,"call.png",true,wPhoneMenu)
	ePhoneNumber = guiCreateEdit(0.2933,0.0929,0.6267,0.0679,"",true,wPhoneMenu)
	
	gPhoneBook = guiCreateGridList(0.0867,0.1679,0.8233,0.6554,true,wPhoneMenu)
	local colName = guiGridListAddColumn(gPhoneBook,"Nazwa",0.6)
	local colNumber = guiGridListAddColumn(gPhoneBook,"Numer",0.4)
	
	local row = guiGridListAddRow(gPhoneBook)
	guiGridListSetItemText(gPhoneBook, row, colName, "Nowy numer", false, false)
	...
	bSettings = guiCreateButton(0.0933,0.0375,0.2467,0.0554,"Ustawienia",true,wPhoneMenu)
	bCancel = guiCreateButton(0.7267,0.0357,0.1600,0.0554,"X",true,wPhoneMenu)
...
function guiConfirmDelete(name, number)
	...
	local sx, sy = guiGetScreenSize() 
	wDelete = guiCreateWindow(sx/2 - 150,sy/2 - 50,300,100,"Usuń wpis", false)
	lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3, "Jestes pewien ze chcesz usunac numer '"..name.."' from your contacts list?",true,wDelete)
	guiLabelSetHorizontalAlign (lQuestion,"center",true)
	bButtonDeleteYes = guiCreateButton(0.1,0.65,0.37,0.23,"Tak",true,wDelete)
	bButtonDeleteNo = guiCreateButton(0.53,0.65,0.37,0.23,"Nie",true,wDelete)
	saveDeleteName = name
...
function guiNewContact()
	...
	wNewContact = guiCreateWindow(x, y, width, height, "Dodaj nowy numer", false)
	
	lName = guiCreateLabel(0.1, 0.2, 0.55, 0.2, "Nazwa:", true, wNewContact)
	fName = guiCreateEdit(0.3, 0.2, 0.55, 0.1, "", true, wNewContact)
	
	lNumber = guiCreateLabel(0.1, 0.4, 0.55, 0.1, "Numer:", true, wNewContact)
	fNumber = guiCreateEdit(0.3, 0.4, 0.55, 0.1, "", true, wNewContact)

	bAddContact = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Dodaj nowy nume", true, wNewContact)
	bCancelContact = guiCreateButton(0.1, 0.775, 0.85, 0.15, "Anuluj", true, wNewContact)
end
...
function showSettingsGui(itemValue)
	...
	wPhoneMenu = guiCreateWindow(sx/2 - 125,sy/2 - 175,250,310,"Ustawienia telefonu",false)
	gRingtones = guiCreateGridList(0.0381,0.1977,0.9153,0.6706,true,wPhoneMenu)
	guiGridListAddColumn(gRingtones,"ringtones",0.85)
	guiGridListSetItemText(gRingtones, guiGridListAddRow(gRingtones), 1, "tryb wibracji", false, false)
