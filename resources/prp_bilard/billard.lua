...
local tables={ 	-- define table positions here
  -- x,y,z, dimension, interior
	{ 1806.6904296875, -1765.5615234375, 13.546875, 0,0}, -- stol na parkingu
	--{ 2488,-1660,13.34, 0,0},
	-- postaw gdzie chcesz :D
 }
...
addEventHandler("onColShapeHit", resourceRoot, function(el,md)
	...
	if st and st=="pot" then
		...
		if ballNames[model] and deleteBall(el, idx) then
			triggerEvent("broadcastCaptionedEvent", source, ballNames[model].." ", 5, 5, true)
		end
		return
	end
...
addCommandHandler("resettable",function(plr,cmd,tn)
	...
	if not tn then
		outputChatBox("Uzyj: /resettable <numer stolu>", plr)
		return
	end
...
--[[
	...
	createBalls(tn)
	triggerEvent("broadcastCaptionedEvent", plr, getPlayerName(plr) .. " ustawia ponownie kule.", 5, 5, true)
end)                                                                   