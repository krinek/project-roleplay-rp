...
function basketHelp(thePlayer, commandName)
	for _,boundryShape in pairs(boundries) do
        if isElementWithinColShape(thePlayer,boundryShape) then
			outputChatBox("/pickball: By podnieć piłke..", thePlayer)
			outputChatBox("/shootball: By rzucić piłke do kosza  / To shoot a ball.", thePlayer)
			outputChatBox("/passball: By rzucić piłke w kierunku, w którym jeste odwrócony /To pass the ball the direction you're facing.", thePlayer)
			outputChatBox("To dunk/Dla nacpanych: biegnij trzymajšc piłke / just run to the basket while holding the ball.", thePlayer)
			outputChatBox("Upewnij sie że znasz dobrze akcje RP / Make sure you always roleplay your actions.", thePlayer)
		end
	end
end
...
function enterBoundries(thePlayer, matchingDimension)
	if getElementType ( thePlayer ) == "player" and matchingDimension then 
		outputChatBox("Witaj w miejscu , w którym możesz pograć w koszykówke , wpisz  /baskethelp by zobaczyć komendy.", thePlayer)
	end
end
