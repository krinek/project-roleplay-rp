...
function removeBackup(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) or (getTeamName(thePlayerTeam) == "Pomoc Drogowa" and getElementData(thePlayer, "factionleader") == 1) then
		if (backupPlayer~=nil) then
			
			for k,v in ipairs(getPlayersInTeam ( getTeamFromName("Pomoc Drogowa") )) do
...
function backup(thePlayer, commandName)
	...
	--if (factionType==3 or getTeamName(thePlayerTeam) == "Pomoc Drogowa") then--Leaving this in in case of abuse.
