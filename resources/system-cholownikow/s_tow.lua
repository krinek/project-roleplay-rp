			...
			if (owner > 0) then
				if (faction > 3 or faction < 0) then
					if (source == towSphere2) then
						...
						exports['antyczit']:changeProtectedElementDataEx(temp, "Impounded", 0)
						exports['antyczit']:changeProtectedElementDataEx(temp, "enginebroke", 0, false)
						setVehicleDamageProof(temp, false)
						setVehicleEngineState(temp, false)
						outputChatBox("((Please remember to /zaparkuj and /reczny your vehicle in our car park.))", thePlayer, 255, 194, 14)
					else
						if (getElementData(temp, "faction") ~= 30) then
							if (impounded == 0) then
								--unlock it and impound it
								exports['antyczit']:changeProtectedElementDataEx(temp, "Impounded", getRealTime().yearday)
								setVehicleLocked(temp, false)
								exports['antyczit']:changeProtectedElementDataEx(temp, "enginebroke", 1, false)
								setVehicleEngineState(temp, false)
				...
				if (owner > 0) then
					if (faction > 3 or faction < 0) then
						if (source == towSphere2) then
							...
							outputChatBox("((Please remember to /zaparkuj and /reczny your vehicle in our car park.))", player, 255, 194, 14)
						else
							if rank >= 5 then
								if (getElementData(veh, "faction") ~= 30) then
									if (impounded == 0) then
										exports['antyczit']:changeProtectedElementDataEx(veh, "Impounded", getRealTime().yearday)
										setVehicleLocked(veh, false)
...
function payRelease(vehID)
	if exports.global:takeMoney(source, 225) then
		exports.global:giveMoney(getTeamFromName("Pomoc Drogowa"), 225)
		...
		exports['antyczit']:changeProtectedElementDataEx(vehID, "enginebroke", 0, false)
		setVehicleDamageProof(vehID, false)
		setVehicleEngineState(vehID, false)
		exports['antyczit']:changeProtectedElementDataEx(vehID, "handbrake", 0, false)
		exports['antyczit']:changeProtectedElementDataEx(vehID, "Impounded", 0)

		outputChatBox("Your vehicle has been released. (( Please remember to /zaparkuj your vehicle so it does not respawn in our carpark. ))", source, 255, 194, 14)
	else
					...
					if commandName == "unimpound" then
						if (getElementData(thePlayer,"faction") == 30) then
							if(getElementData(thePlayer, "factionleader") == 1) then
								...
								local theTeam = getTeamFromName("Pomoc Drogowa")
...
function updateTowingVehicle(theTruck)
	...
	if (thePlayer) then
		if (getElementData(thePlayer,"faction") == 30) then
			...
			if (handbrake == 1) then
				exports['antyczit']:changeProtectedElementDataEx(source, "handbrake",0,false)
