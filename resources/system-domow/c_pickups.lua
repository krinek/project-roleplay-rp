...
function applyPickupClientConfigSettings()
	local streamerDistance = tonumber( exports['system-kont']:loadSavedData("streamer-pickup", "25") )
...
if debugmode then
	...
	local pickupid = int[INTERIOR_DISABLED] and 1314 or ( getElementType(interiorElement) == "elevator" and 1559 or ( int[INTERIOR_TYPE] == 2 and 1559 or ( int[INTERIOR_OWNER] < 1 and ( int[INTERIOR_TYPE] == 1 and 1272 or 1273 ) or 1559 ) ) )
	local outsidePickup = nil
	if pickupid ~= 1559 then
		outsidePickup = createPickup( entrance[INTERIOR_X], entrance[INTERIOR_Y], entrance[INTERIOR_Z], 3, int[INTERIOR_DISABLED] and 1314 or ( getElementType(interiorElement) == "elevator" and 1559 or ( int[INTERIOR_TYPE] == 2 and 1559 or ( int[INTERIOR_OWNER] < 1 and ( int[INTERIOR_TYPE] == 1 and 1272 or 1273 ) or 1559 ) ) ) )
	else
		outsidePickup = createMarker( entrance[INTERIOR_X], entrance[INTERIOR_Y], entrance[INTERIOR_Z]+0.65, "arrow", 1, 255, 255, 0 )
	end
	...
	--local insidePickup = createPickup( exit[INTERIOR_X], exit[INTERIOR_Y], exit[INTERIOR_Z], 3,  1559 )
	local insidePickup = createMarker( exit[INTERIOR_X], exit[INTERIOR_Y], exit[INTERIOR_Z]+0.65, "arrow", 1, 255, 255, 0 )
...
function hitInteriorPickup(theElement, matchingdimension)
	...
	if getElementType(colshape) == "interior" or getElementType(colshape) == "elevator" then
		...
		if matchingdimension and (theElement == getLocalPlayer() or isVehicle) and ((not theVehicle and getElementType(colshape) == "interior") or getElementType(colshape) == "elevator") then
			if getElementType(colshape) == "interior" or getElementType(colshape) == "elevator" then
				lastSource = false
...
function showLagCatcher()
	...
	--lagCatcherWindow = guiCreateStaticImage( x*.5-150,0, 400, 190, "info.png", false )
	--lagCatcherMessage = guiCreateLabel( 97, 32, 220, 60, "Serwer wysyła wszystkie\ninteriory do gry\nZaczekaj chwilkę aż proces sie zakończy.\n", false, lagCatcherWindow )
...
function updateLagCatcher()
	if lagcatcherenabled then
		...
		--local str = "Proces: "..tostring(done).."/"..tostring(total)
		--guiSetText(lagCatcherMessage, "Serwer instaluje interiory,\nProszę czekać. Potrwa to do \nParu sekund.\n\n"..str)
	end
end
