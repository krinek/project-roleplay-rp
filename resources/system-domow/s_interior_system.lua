...
function setElementDataEx(source, field, parameter, streamtoall, streamatall)
	exports['antyczit']:changeProtectedElementDataEx( source, field, parameter, streamtoall, streamatall)
end
-- End small hack
...
function cleanupProperty( id, donotdestroy )
	if id > 0 then
		if exports.mysql:query_free( "DELETE FROM dancers WHERE dimension = " .. mysql:escape_string(id) ) then
			local res = getResourceRootElement( getResourceFromName( "system-taniec" ) )
			if res then
		...
		if exports.mysql:query_free( "DELETE FROM shops WHERE dimension = " .. mysql:escape_string(id) ) then
			local res = getResourceRootElement( getResourceFromName( "sklepy" ) )
			if res then
				for key, value in pairs( getElementsByType( "ped", res ) ) do
					if getElementDimension( value ) == id then
						destroyElement( value )
		...
		if exports.mysql:query_free( "DELETE FROM atms WHERE dimension = " .. mysql:escape_string(id) ) then
			local res = getResourceRootElement( getResourceFromName( "system-bank" ) )
			if res then
		...
		if not donotdestroy then
			local res = getResourceRootElement( getResourceFromName( "system-obiektow" ) )
			if res then
				exports['system-obiektow']:removeInteriorObjects( tonumber(id) )
			end
		end
		
		if safeTable[id] then
			local safe = safeTable[id]
			call( getResourceFromName( "system-przedmiotow" ), "clearItems", safe )
			destroyElement(safe)
			safeTable[id] = nil
		end
	end
...
function sellProperty(thePlayer, commandName, bla)
	if bla then
		outputChatBox("uzyj komendy /sdom by sprzedać dom.", thePlayer, 255, 0, 0)
		return
	end
	...
	if dbid > 0 then
		if interiorType == 2 then
			outputChatBox("Nie da sie sprzedac domu urzędowego.", thePlayer, 255, 0, 0)
		elseif interiorType ~= 3 and commandName == "unrent" then
			outputChatBox("Nie wynajmujesz tego mieszkania.", thePlayer, 255, 0, 0)
		else
			local interiorStatus = getElementData(interiorElement, "status")
			if exports.global:isPlayerLeadAdmin(thePlayer) or exports.sponsorzy:hasPlayerPerk(thePlayer, 14) or interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
				...
				outputChatBox("Ten budynek nie jest twój.", thePlayer, 255, 0, 0)
			end
		end
	else 
...
addCommandHandler("zakonczwynajem", sellProperty, false, false)
...
function publicSellProperty(thePlayer, dbid, showmessages, givemoney)
	...
	if query then
		...
		if getElementDimension(thePlayer) == dbid then
			...
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "interiormarker", false, false, false)
		end
		...
		if interiorType == 0 or interiorType == 1 then
			if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
				...
				if showmessages then
					outputChatBox("Sprzedałeś dom za " .. exports.global:formatMoney(money) .. "$.", thePlayer, 0, 255, 0)
				end
				
				-- take all keys
				call( getResourceFromName( "system-przedmiotow" ), "deleteAll", 4, dbid )
				call( getResourceFromName( "system-przedmiotow" ), "deleteAll", 5, dbid )
...
function sellTo(thePlayer, commandName, targetPlayerName)
	...
	if dbid > 0 and not isPedInVehicle( thePlayer ) then
		...
		elseif not targetPlayerName then
			...
			outputChatBox("Sprzedawanie domu innemu graczowi.", thePlayer, 255, 194, 14)
			outputChatBox("Przed sprzedaniem porpos kupującego o przelew bankowy lub przekazanie pieniędzy.", thePlayer, 255, 194, 14)
		else
...
addCommandHandler("sdom", sellTo)

function realReloadInterior(interiorID)
	local dbid, entrance, exit, inttype, interiorElement = exports['system-domow']:findProperty( false, interiorID )
	if dbid > 0 then
...
function buyInterior(player, pickup, cost, isHouse, isRentable)
	...
	if exports.global:takeMoney(player, cost) then
		if (isHouse) then
			outputChatBox("Gratulacje , właśnie zakupiłeś dom za $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("Government of Los Santos"), cost )
		elseif (isRentable) then
			outputChatBox("Gratulacje , wynajmujesz od teraz miejsce za $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Gratulacje , zakupiłeś bisnez za $" .. exports.global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("Government of Los Santos"), cost )
		end
		...
		call( getResourceFromName( "system-przedmiotow" ), "deleteAll", 4, pickupid )
		call( getResourceFromName( "system-przedmiotow" ), "deleteAll", 5, pickupid )
		...
		outputChatBox("Nie masz pieniędzy by zakupić ten budynek..", player, 255, 194, 14)
	end
end
						...
						for k, v in pairs( getElementsByType( "player" ) ) do
							if isElement( v ) then
								if getElementData( v, "dbid" ) == ownerid then
									exports['antyczit']:changeProtectedElementDataEx( v, "businessprofit", getElementData( v, "businessprofit" ) + fee, false )
									break
								end
							end
						end
...
function addSafeAtPosition( thePlayer, x, y, z, rotz )
	...
	elseif dbid >= 20000 then -- Vehicle Interiors
		...
		if exports['system-aut']:getSafe( vid ) then
		...
		elseif exports.global:hasItem( thePlayer, 3, vid ) then
			...
			if exports.mysql:query_free( "UPDATE vehicles SET safepositionX='" .. x .. "', safepositionY='" .. y .. "', safepositionZ='" .. z .. "', safepositionRZ='" .. rotz .. "' WHERE id='" .. vid .. "'") then
				if exports['system-aut']:addSafe( vid, x, y, z, rotz, interior ) then
					return 0
				end
			end
			return 1
	...
	elseif ((exports.global:hasItem( thePlayer, 5, dbid ) or exports.global:hasItem( thePlayer, 4, dbid))) then
		if safeTable[dbid] then
			...
			call( getResourceFromName( "system-przedmiotow" ), "clearItems", tempobject )
			return 0
		end
	end
	return 3
...
function moveSafe ( thePlayer, commandName )
	...
	if (dbid < 19000 and (exports.global:hasItem( thePlayer, 5, dbid ) or exports.global:hasItem( thePlayer, 4, dbid))) or (dbid >= 20000 and exports.global:hasItem(thePlayer, 3, dbid - 20000)) then
		z = z - 0.5
		rotz = rotz + 180
		if dbid >= 20000 and exports['system-aut']:getSafe(dbid-20000) then
			local safe = exports['system-aut']:getSafe(dbid-20000)
...
function lockUnlockHouseEvent(player, checkdistance)
	...
	if found and itemValue then
		...
		if locked == 0 then
			...
			exports.global:sendLocalMeAction(source, "wkłada klucz do drzwi po czym je otwiera.")
			exports.logs:dbLog(source, 31, {  "in"..tostring(itemValue) }, "UNLOCK INTERIOR")
		else
			newRealLockedValue = true
			exports.global:sendLocalMeAction(source, "wkłada klucz do drzwi po czym je zamyka.")
...
function client_requestHUDinfo()
	...
	exports['antyczit']:changeProtectedElementDataEx( client, "interiormarker", true, false, false )
...
addEventHandler("int:updatemarker", getRootElement(), 
	function( newState )
		exports['antyczit']:changeProtectedElementDataEx(client, "interiormarker", newState, false, true) -- No sync at all: function is only called from client thusfar has the actual state itself
	end
)