			...
			if (result2) then
				...
				if (result3) then
					...
					if (result4) and (result5) then
						exports['system-domow']:reloadInterior(thePlayer, "reloadinterior", tostring(dimension))
...
function deleteInterior(thePlayer, commandName, dimensionID)
	if exports.global:isPlayerLeadAdmin(thePlayer) or exports.sponsorzy:hasPlayerPerk(thePlayer, 13) then
		if dimensionID and  tonumber(dimensionID) then
			loadDimension(tonumber(dimensionID))
			exports['system-domow']:reloadInterior(thePlayer, "reloadinterior", dimensionID)
