...
function controlRamp(theVehicle)
	...
	if not (exports['system-przedmiotow']:hasItem(theVehicle, 117)) then
	...
	if not (rampObject) or not (isElement(rampObject)) then
		if (getElementModel(theVehicle) == 578) then
			...
			exports['antyczit']:changeProtectedElementDataEx(theVehicle, "vehicle:ramp:object", rampObject, false)
		end
	else
		destroyElement(rampObject)
		exports['antyczit']:changeProtectedElementDataEx(theVehicle, "vehicle:ramp:object", nil, false)
	end
end
