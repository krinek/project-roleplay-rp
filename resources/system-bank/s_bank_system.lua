...
function withdrawMoneyPersonal(amount)
	...
	if money >= 0 then
		...
		outputChatBox("Wypłaciłeś $" .. exports.global:formatMoney(amount) .. " ze swojego konta bankowego.", client, 255, 194, 14)
...
function depositMoneyPersonal(amount)
	...
	if exports.global:takeMoney(client, amount) then
		...
		exports['antyczit']:changeProtectedElementDataEx(client, "bankmoney", money+amount, false)
		saveBank(client)
		...
		outputChatBox("Wpłaciłeś $" .. exports.global:formatMoney(amount) .. " do banku.", client, 255, 194, 14)
...
function withdrawMoneyBusiness(amount)
	...
	if exports.global:takeMoney(theTeam, amount) then
		if exports.global:giveMoney(client, amount) then
			...
			outputChatBox("Wypłaciłeś $" .. exports.global:formatMoney(amount) .. " z konta frakcji.", client, 255, 194, 14)
...
function depositMoneyBusiness(amount)
	...
	if exports.global:takeMoney(client, amount) then
		...
		if exports.global:giveMoney(theTeam, amount) then
			...
			outputChatBox("Wpłaciłeś $" .. exports.global:formatMoney(amount) .. " do konta frakcji.", client, 255, 194, 14)
	...
	if not dbid and not reciever then
		...
		if business then
			...
			if getElementData(client, "bankmoney") - amount >= 0 then
				exports['antyczit']:changeProtectedElementDataEx(client, "bankmoney", getElementData(client, "bankmoney") - amount, false)
		...
		if reciever then
			if dbid < 0 then
				...
				exports['antyczit']:changeProtectedElementDataEx(reciever, "bankmoney", getElementData(reciever, "bankmoney") + amount, false)
				saveBank(reciever)
			end
		else
...
--[[
	...
	7: everything in wyplata except Wage/State Benefits
	8: faction budget
]]
...
function tellTransfers(source, dbid, event)
	...
	if dbid < 0 then
		where = where .. " AND type != 6" -- skip wyplatas for factions 
	else
