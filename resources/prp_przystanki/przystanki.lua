...
--[[
    ...
    sannews={479.58,-1084.71,82.48,356.5-90, nazwa="SAN News" },
...
--	    outputChatBox("row " .. v.row .. " nazwa " .. v.nazwa)
		...
		x,y,z = getElementPosition(localPlayer)
			...
			--setElementPosition(localPlayer, 2021.93, 2236.74, 2110.95)
				...
				setTimer(setElementFrozen, 1000, 1, localPlayer, true)
				...
				if exports.premium:isPlayerPremium() then
					czas_jazdy = czas_jazdy/2
				end
				
				time = tostring(czas_jazdy):explode("%.")
				sekundy = tonumber(string.sub(time[2],1,1)) or 0
				if sekundy >= 10 then
					sekundy = sekundy/10
				end
				sekundy = math.floor(sekundy*6)
				sekundy = tostring(sekundy) or 0
				...
				--fadeCamera(false)
				setTimer(fadeCamera, 200, 1, false)
				--sound = playSound("autobus.ogg",true)
				text = createElement("text")
				setElementPosition(text,2021.93, 2236.74, 2103.95)
				--setTimer(setElementPosition, 2000, 1, text, 2021.93, 2236.74, 2103.95)
				setElementData(text, "scale", 1.5)
				setElementData(text, "text", "Czas podróży: xxx")
				...
				setTimer(function()
					...
					if pozostalo_s >= 0 and pozostalo_m >= 0 then
						local s = pozostalo_s
						if pozostalo_s < 10 then s = "0"..s end
						local m = pozostalo_m
						if pozostalo_m < 10 then m = "0"..m end
						setElementData(text, "text", "Czas podróży: "..m..":"..s)
					end
					if pozostalo_s <= 0 then pozostalo_s=60 pozostalo_m=pozostalo_m-1 end
					setElementPosition(text, 2021.93, 2236.74, 2103.95)
