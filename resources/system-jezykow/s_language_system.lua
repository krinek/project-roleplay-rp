	...
	if (hasLanguage) then
		...
		if currSkill < 100 then
			...
			if chance == 1 then
				...
				exports['antyczit']:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", currSkill+1, false)
...
function removeLanguage(player, language)
	...
	if (hasLanguage) then
		...
		exports['antyczit']:changeProtectedElementDataEx(player, "languages.lang" .. slot, 0, false)
		exports['antyczit']:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", 0, false)
...
function learnLanguage(player, lang, showmessages, skill)
	...
	if (hasLanguage) then
		...
		if skill then
			exports['antyczit']:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", skill, false)
		...
		if (freeslot==0) then
			...
			exports['antyczit']:changeProtectedElementDataEx(player, "languages.lang" .. freeslot, lang, false)
			exports['antyczit']:changeProtectedElementDataEx(player, "languages.lang" .. freeslot .. "skill", skill or 0, false)
...
function useLanguage(lang)
	...
	if (hasLanguage) then
		...
		exports['antyczit']:changeProtectedElementDataEx(source, "languages.current", slot, false)
...
function unlearnLanguage(lang)
	...
	if (hasLanguage) then
		...
		exports['antyczit']:changeProtectedElementDataEx(source, "languages.current", nextSlot, false)
...
for key, value in pairs( languages ) do
	function speakLang( thePlayer, commandName, ... )
		if doesPlayerHaveLanguage( thePlayer, key ) then
			call( getResourceFromName( "czat" ), "localIC", thePlayer, table.concat({...}, " "), key )
		else
