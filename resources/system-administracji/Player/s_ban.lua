...
function banAPlayer(thePlayer, commandName, targetPlayer, hours, ...)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (targetPlayer) or not (hours) or (tonumber(hours)<0) or not (...) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Częściowy nick / ID] [Czas w godzinach, 0 = Nieskończoność] [Powód]", thePlayer, 255, 194, 14)
		else
			...
			elseif (hours>168) then
				outputChatBox("Nie możesz zbanować na więcej niż 7 dni (168 Godzin).", thePlayer, 255, 194, 14)
			else
				...
				if (targetPlayerPower <= thePlayerPower) then -- Check the admin isn't banning someone higher rank them him
					...
					if (hiddenAdmin==0) then
						...
						--outputChatBox("AdmBan: " .. adminTitle .. " " .. playerName .. " zbanował gracza " .. targetPlayerName .. ". (" .. hours .. ")", getRootElement(), 255, 0, 51)
						--outputChatBox("AdmBan: Powód: " .. reason .. ".", getRootElement(), 255, 0, 51)
						local okienkoreason = "Nadawca: " .. adminTitle .. " " .. playerName
						okienkoreason = okienkoreason .. "\n"
						okienkoreason = okienkoreason .. "\n"
						okienkoreason = okienkoreason .. "Gracz: " .. targetPlayerName
						okienkoreason = okienkoreason .. "\n"
						okienkoreason = okienkoreason .. "\n"
						okienkoreason = okienkoreason .. "Powód: " .. reason2
						--local okienkoreason = adminTitle .. " " .. playerName .. " zbanował gracza " .. targetPlayerName .. ". (" .. hours .. ")"
						--okienkoreason = okienkoreason .. "\n" .. "Powód: " .. reason .. "."
						triggerClientEvent( "doOkienkoBan", getRootElement(), "BAN (".. hours..")", okienkoreason )					
					...
					elseif (hiddenAdmin==1) then
						--outputChatBox("AdmBan: Ukryty Admin zbanował " .. targetPlayerName .. ". (" .. hours .. ")", getRootElement(), 255, 0, 51)
						--outputChatBox("AdmBan: Powód: " .. reason, getRootElement(), 255, 0, 51)
						...
						okienkoreason = okienkoreason .. "\n"
						okienkoreason = okienkoreason .. "Powód: " .. reason2
						--local okienkoreason = "Ukryty Admin zbanował " .. targetPlayerName .. ". (" .. hours .. ")"
						--okienkoreason = okienkoreason .. "\n" .. "Powód: " .. reason .. "."
						triggerClientEvent( "doOkienkoBan", getRootElement(), "BAN (".. hours..")", okienkoreason )	
					...
					outputChatBox(" Ten gracz posiada wyższy level admina od ciebię :).", thePlayer, 255, 0, 0)
					outputChatBox(playerName .. " próbował wykonać komendę ban na tobie.", targetPlayer, 255, 0 ,0)
				end
			end
		end
	end
...
function unbanPlayer(thePlayer, commandName, ...)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Nazwa Użytkownika/IP/Serial]", thePlayer, 255, 194, 14)
		else
			...
			if not accountID then
				outputChatBox("Nie znaleziono bana dla '" .. searchString .. "'", thePlayer, 255, 0, 0)
				return
			end
			
			if (accountID == -1 and localBan) then
				exports.global:sendMessageToAdmins("[Zbanowani] "..getBanNick(localBan) .. "/"..getBanSerial(localBan).." został odbanowany przez " .. getPlayerName(thePlayer) .. ". (S: ".. searchCode ..")")
				removeBan( localBan )
				return
			end
			...
			if banDetails then 
				...
				if not (unbannedSomething) and not (banDetails["banned"] == 1) then
					...
					exports.global:sendMessageToAdmins("[Zbanowani] "..banDetails["username"] .. "/"..banDetails["mtaserial"].."/".. banDetails["id"] .." został odbanowany przez " .. getPlayerName(thePlayer) .. ". (S: ".. searchCode ..")")
...
function unbanPlayer(thePlayer, commandName, ...)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Player Full Name]", thePlayer, 255, 194, 14)
		else
			local bans = getBans()
			local found = false
				...
				if (result) then
					if (mysql:num_rows(result)>0) then
						...
						for key, value in ipairs(bans) do
							...
							-	exports.global:sendMessageToAdmins(tostring(nickName) .. " został odbanowany przez " .. getPlayerName(thePlayer) .. ".")
...
function unbanPlayerIP(thePlayer, commandName, ip)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (ip) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [IP]", thePlayer, 255, 194, 14)
		else
			...
			for key, value in ipairs(bans) do
				if (ip==getBanIP(value)) then
					exports.global:sendMessageToAdmins(tostring(ip) .. " zostaje odbanowany przez " .. getPlayerName(thePlayer) .. ".")
					removeBan(value, thePlayer)
					mysql:query_free("UPDATE accounts SET banned='0', banned_by=NULL WHERE ip='" .. mysql:escape_string(ip) .. "'")
					found = true
					--break
			...
			if found then
				outputChatBox("Odbanowano  ip '" .. ip .. "'", thePlayer, 255, 0, 0)
			else
				outputChatBox("Nie ma bana pod adresem  '" .. ip .. "'", thePlayer, 255, 0, 0)
			end
		end
	end
end
...
function unbanPlayerSerial(thePlayer, commandName, ip)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (ip) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Serial]", thePlayer, 255, 194, 14)
		else
			ip = mysql:escape_string(ip)
			local bans = getBans()
			local found = false
			...
			if found then
				outputChatBox("Odbanowanio IP '" .. ip .. "'", thePlayer, 255, 0, 0)
			else
				outputChatBox("Nie znaleziono bana na ip '" .. ip .. "'", thePlayer, 255, 0, 0)
			end
		end
	end
end
...
function checkForFairPlayBan(dummy1, dummy2, dummy3, MTAserial)
	...
	if exports.mysql:num_rows(result) > 0 then
		...
		exports.global:sendMessageToAdmins("[MTA.RP] Wykryto próbę połączneia " .. row["username"] .. "/" .. MTAserial .." ten user ma bana :).")
