		...
		if exports.global:isPlayerAdmin( localPlayer ) or (getElementData(localPlayer, "account:gmlevel") > 0 and getElementData(localPlayer, "account:gmduty")) then
			...
			if not name then
				if default then
					...
					outputChatBox( "Muszisz najpierw ustaliæ /mark !", 255, 0, 0 )
				end
			else
				if marks[ name ] then
					triggerServerEvent( "gotoMark", localPlayer, unpack( marks[ name ] ) )
				else
					outputChatBox( "Musisz najpierw ustaliæ mark komenda /mark " .. name .. " :) !", 255, 0, 0 )
				end
			end
		end
	end
...
addCommandHandler( "marki",
	function( commandName )
		local count = 1
		if default then
			outputChatBox( "  #" .. count .. ": (domyœlnny)", 255, 255, 0 )
			count = count + 1
		end
	...
	function( commandName, ... )
		if exports.global:isPlayerAdmin( localPlayer ) or (getElementData(localPlayer, "account:gmlevel") > 0 and getElementData(localPlayer, "account:gmduty")) then
			...
			if not name then
				if default then
					default = false
					outputChatBox( "Domyœlnny mark zosta³ usuniêty.", 0, 255, 0 )
					saveMarks( )
				else
					outputChatBox( "Musisz ustaliæ mark przez /mark najpierw!", 255, 0, 0 )
				end
			else
				if marks[ name ] then
					marks[ name ] = nil
					outputChatBox( "Mark '" .. name .. "' poprawnie usuniêty.", 0, 255, 0 )
					saveMarks( )
				else
					outputChatBox( "Musisz ustaliæ mark przez /mark " .. name .. " najpierw!", 255, 0, 0 )
				end
			end
		end
	end
