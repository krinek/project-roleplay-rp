		...
		if not (target) then
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					outputChatBox(" Teleportowałeś się do gracza " .. targetPlayerName .. ".", thePlayer)
					if exports.global:isPlayerGameMaster(thePlayer) then
						outputChatBox(" Technik RP " .. username .. " Przeteleportował się do ciebie. ", targetPlayer)
					else
						outputChatBox(" Admin " .. username .. " Przeleleportował się do ciebie. ", targetPlayer)
					end
				end
			end
		end
...
function getPlayer(thePlayer, commandName, from, to)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if(not from or not to) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Gracz przesyłany] [Gracz docelowy]", thePlayer, 255, 194, 14)
		else
			...
			if(fromplayer and toplayer) then
				...
				if(not logged1 or not logged2) then
					outputChatBox("Jeden z graczy jest niezalogowany.", thePlayer, 255, 0 , 0)
				else
					...
					outputChatBox(" Teleportowałeś " .. targetPlayerName1:gsub("_"," ") .. " do " .. targetPlayerName2:gsub("_"," ") .. ".", thePlayer)
					outputChatBox(" Administrator " .. admin .. " teleportował cię do " .. targetPlayerName2:gsub("_"," ") .. ". ", fromplayer)
					outputChatBox(" Administrator " .. admin .. " teleportował " .. targetPlayerName1:gsub("_"," ") .. " do ciebie.", toplayer)
				end
			end
		end
	end
...
function getPlayer(thePlayer, commandName, target)
	if (exports.global:isPlayerAdmin(thePlayer) or exports.global:isPlayerFullGameMaster(thePlayer)) then
		...
		if not (target) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " /gethere [Częściowy nick]", thePlayer, 255, 194, 14)
		else
			local username = getPlayerName(thePlayer)
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, target)
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					outputChatBox(" Przeteleportowałeś gracza " .. targetPlayerName .. " do siebię.", thePlayer)
					if exports.global:isPlayerFullGameMaster(thePlayer) then
						outputChatBox(" Gamemaster " .. username .. " teleportował ciebię do niego. ", targetPlayer)
					else
						outputChatBox(" Admin " .. username .. " teleportował ciebię do niego. ", targetPlayer)
					end
				end
			end
		end
...
function teleportToPresetPoint(thePlayer, commandName, target)
	if (exports.global:isPlayerAdmin(thePlayer) or exports.global:isPlayerGameMaster(thePlayer)) then
		if not (target) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [Miejsce]", thePlayer, 255, 194, 14)
		else
			...
			if (teleportLocations[target] ~= nil) then
				...
				outputChatBox("Podano złe miejsce!", thePlayer, 255, 0, 0)
			end
		end
	end
end
		...
		if type( x ) == "number" and type( y ) == "number" and type( z ) == "number" and type( interior ) == "number" and type( dimension ) == "number" then
			if getElementData ( client, "loggedin" ) == 1 and ( exports.global:isPlayerAdmin(client) or exports.global:isPlayerGameMaster(client) ) then
				...
				if(vehicle and (seat ~= 0)) then
					...
					exports['antyczit']:changeProtectedElementDataEx(client, "realinvehicle", 0, false)
