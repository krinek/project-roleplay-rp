...
function getAdminTitle(thePlayer)
	...
	local text = ({ "Admin Testowy", "Admin", "Super Admin", "Lead Admin", "Head Admin", "RCON", nil, nil, nil, "Skrypter" })[adminLevel] or "Player"
...
local GMtitles = { "Testowy Support", "Support", "Ekspert - Support", "Zarządca supportu" }
...
local function updateGUI()
	if show then
		...
		textString = getAdminTitle( localPlayer ) .. " :: " .. onduty .. admstr .. ( (getElementData( localPlayer, "adminlevel" ) <= 6 )and ( " :: " .. ( openReports - handledReports ) .. " zgłoszeń oczekujących" .. reporttext .. " :: " .. handledReports .. " zgłoszeń otwartych" .. ownreporttext ) or "" )
	end
end
