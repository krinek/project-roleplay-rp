function doCheck(sourcePlayer, command, ...)
	if (exports.global:isPlayerAdmin(sourcePlayer)) then
		if not (...) then
			outputChatBox("PRZYKŁAD: /" .. command .. " [Częściowy Nick / ID]", sourcePlayer, 255, 194, 14)
		else
			...
			if (noob) then
				...
				if (logged==0) then
					outputChatBox("Gracz nie jest zalogowany.", sourcePlayer, 255, 0, 0)
				else
	...
	if exports.global:isPlayerAdmin( client ) then
		...
		if targetID then
			...
			if result then
				...
				outputChatBox( "Błąd czytania histori.", client, 255, 0, 0)
			end
		else
			outputChatBox("Nie można znaleźć id konta.", client, 255, 0, 0)
		end
	end
end
...
function showOfflineAdminHistory( gameaccountid, name )
	if exports.global:isPlayerAdmin( source ) and tonumber(gameaccountid) then
		...
		if result then
			...
			outputDebugString( "system-administracji\showOfflineAdminHistory: Error." )
...
function removeAdminHistoryLine(ID, target)
	...
	if exports.global:isPlayerHeadAdmin(client) then -- type 4
		...
		outputChatBox("Niedostępne", client, 255, 0, 0)
	end
end
		...
		if exports.global:isPlayerAdmin( thePlayer ) then
			if not (...) then
				...
				if targetPlayer then
					...
					mysql:free_result( result )
					outputChatBox("Gracz nie został znaleziony, lub znaleziono kilka podobnych.", thePlayer, 255, 0, 0)
				end
			end
		end
	end
