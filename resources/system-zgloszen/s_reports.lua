...
function resourceStart(res)
	for key, value in ipairs(exports.pool:getPoolElementsByType("player")) do
		exports['antyczit']:changeProtectedElementDataEx(value, "report", false, false)
		exports['antyczit']:changeProtectedElementDataEx(value, "reportadmin", false, false)
	end	
end
...
function reportInfo(thePlayer, commandName, id)
	if (exports.global:isPlayerAdmin(thePlayer) or exports.global:isPlayerGameMaster(thePlayer)) then
		if not (id) then
			outputChatBox("PRZYK£AD: " .. commandName .. " [ID]", thePlayer, 255, 194, 15)
		else
			id = tonumber(id)
			if reports[id] then
			...
			if reports[i] then
				if reports[i][5] == source then
					...
					for key, value in ipairs(exports.global:getGameMasters()) do
						local adminduty = getElementData(value, "account:gmduty")
						if adminduty == true then
							outputChatBox(" [GM#" .. i .."] Report is unassigned (" .. getPlayerName(source) .. " wyszed³ z gry)", value, 0, 255, 255)
							update = true
						end
					end
				end
				if reports[i][2] == source then
					for key, value in ipairs(exports.global:getGameMasters()) do
						local adminduty = getElementData(value, "account:gmduty")
						if adminduty == true then
							outputChatBox(" [GM#" .. i .."] Zg³aszaj¹cy " .. getPlayerName(source) .. " wyszed³ z gry.", value, 0, 255, 255)
							update = true
						end
					end
					...
					if reporter ~= source then
						outputChatBox("Twoje zg³oszenie GM#" .. i .. " zosta³o zamkniête (" .. getPlayerName(source) .. " wyszed³ z gry)", reporter, 255, 194, 14)
						exports['antyczit']:changeProtectedElementDataEx(reporter, "report", false, false)
						exports['antyczit']:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
					end
...
function handleReport(reportedPlayer, reportedReason, isGMreport)
	...
	exports['antyczit']:changeProtectedElementDataEx(source, "report", slot, false)
	exports['antyczit']:changeProtectedElementDataEx(source, "reportadmin", false)
	local count = 0
	local skipadmin = false
	if isGMreport then
		exports['antyczit']:changeProtectedElementDataEx(source, "gmreport", slot, false)
...
function pendingReportTimeout(id)
	if (reports[id]) then
		...
		exports['antyczit']:changeProtectedElementDataEx(reportingPlayer, "reportadmin", false, false)
		...
		if isGMreport then
			exports['antyczit']:changeProtectedElementDataEx(reportingPlayer, "gmreport", false, false)
			local GMs = exports.global:getGameMasters()
			for key, value in ipairs(GMs) do
				local gmduty = getElementData(value, "account:gmduty")
				if (gmduty== true) then
			...
			exports['antyczit']:changeProtectedElementDataEx(reportingPlayer, "report", false, false)
			local admins = exports.global:getAdmins()
			-- Show to admins
			for key, value in ipairs(admins) do
				local adminduty = getElementData(value, "adminduty")
				if (adminduty==1) then
					outputChatBox(" [#" .. id .. "] - REPORT #" .. id .. " wygas³o!", value, 0, 255, 255)
				end
			end
		end
		
		outputChatBox("[" .. timestring .. "] Your report (#" .. id .. ") has expired.", reportingPlayer, 255, 194, 14)
		outputChatBox("[" .. timestring .. "] JeÅ¼ei. nie otrzymasz odpowiedzi uda siedo naszej strony w clu napisania tematu na www.california-gaming.", reportingPlayer, 255, 194, 14)
...
function falseReport(thePlayer, commandName, id)
	if (exports.global:isPlayerAdmin(thePlayer) or exports.global:isPlayerGameMaster(thePlayer)) then
		if not (id) then
			outputChatBox("PRZYK£AD: /" .. commandName .. " [Report ID]", thePlayer, 255, 194, 14)
		else
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					exports['antyczit']:changeProtectedElementDataEx(thePlayer, "adminreports", adminreports+1, false)
					...
					exports['antyczit']:changeProtectedElementDataEx(reportingPlayer, "reportadmin", thePlayer, false)
					
					local timestring = hours .. ":" .. minutes
					local playerID = getElementData(reportingPlayer, "playerid")
					
					if (exports.global:isPlayerAdmin(thePlayer)) then
						outputChatBox("[" .. timestring .. "] Administrator " .. getPlayerName(thePlayer) .. " zaakceptowa³ twoje zg³oszenie (#" .. id .. "), Poczekaj a¿ skontaktuje sie z tob¹.", reportingPlayer, 255, 194, 14)
						executeCommandHandler("check", thePlayer, tostring(playerID))
					else
						outputChatBox("[" .. timestring .. "] Gamemaster " .. getPlayerName(thePlayer) .. " zaakceptowa³ twoje zg³oszenie (#" .. id .. "), Poczekaj a¿ skontaktuje sie z tob¹.", reportingPlayer, 255, 194, 14)
					end
					...
					if isGMreport then
						...
						for key, value in ipairs(admins) do
							local adminduty = getElementData(value, "account:gmduty")
							if (adminduty==true) then
								outputChatBox(" [GM#" .. id .. "] - " .. getPlayerName(thePlayer) .. " zaakceptowa³ zg³oszenie #" .. id .. " -", value, 0, 255, 255)
							end
						end
					else
						local admins = exports.global:getAdmins()
						for key, value in ipairs(admins) do
							local adminduty = getElementData(value, "adminduty")
							if (adminduty==1) then
								outputChatBox(" [#" .. id .. "] - " .. getPlayerName(thePlayer) .. " zaakceptowa³ zg³oszenie #" .. id .. " -", value, 0, 255, 255)
							end
						end					
					end
...
function acceptAdminReport(thePlayer, commandName, id, ...)
	...
	if (exports.global:isPlayerHeadAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("PRZYK£AD: /" .. commandName .. " [Report ID] [Adminname]", thePlayer, 255, 194, 14)
		else
			...
			if targetAdmin then
				...
				if not (reports[id]) then
					...
					if (reportHandler) then
						...
						exports['antyczit']:changeProtectedElementDataEx(targetAdmin, "adminreports", adminreports+1, false)
						...
						exports['antyczit']:changeProtectedElementDataEx(reportingPlayer, "reportadmin", targetAdmin, false)
						
						local timestring = hours .. ":" .. minutes
						local playerID = getElementData(reportingPlayer, "playerid")
						
						if exports.global:isPlayerGameMaster(targetAdmin) then
							outputChatBox("[" .. timestring .. "] Gamemaster " .. getPlayerName(targetAdmin) .. " zaakceptowa³ twoje zg³oszenie (#" .. id .. "), Zaczekaj a¿ sie z tob¹ skontaktuje.", reportingPlayer, 255, 194, 14)
						else
							outputChatBox("[" .. timestring .. "] Administrator " .. getPlayerName(targetAdmin) .. " zaakceptowa³ twoje zg³oszenie (#" .. id .. "), Zaczekaj a¿ sie z toba skontaktuje.", reportingPlayer, 255, 194, 14)
						end
						...
						if isGMreport then
							...
							for key, value in ipairs(admins) do
								local adminduty = getElementData(value, "account:gmduty")
								if (adminduty==true) then
									outputChatBox(" [GM#" .. id .. "] - " .. getPlayerName(theAdmin) .. " zaakceptowa³ zg³oszenie #" .. id .. " (Assigned) -", value, 0, 255, 255)
								end
							end
						else
							local admins = exports.global:getAdmins()
							for key, value in ipairs(admins) do
								local adminduty = getElementData(value, "adminduty")
								if (adminduty==1) then
									outputChatBox(" [#" .. id .. "] - " .. getPlayerName(theAdmin) .. " zaakceptowa³ zg³oszenie #" .. id .. " (Assigned) -", value, 0, 255, 255)
								end
							end
						end
...
function closeReport(thePlayer, commandName, id)
	if (exports.global:isPlayerAdmin(thePlayer) or exports.global:isPlayerGameMaster(thePlayer)) then
		if not (id) then
			outputChatBox("PRZYK£AD: " .. commandName .. " [ID]", thePlayer, 255, 195, 14)
		else
			id = tonumber(id)
			...
			elseif (reports[id][5] ~= thePlayer) then
				...
				if (isElement(reporter)) then
					exports['antyczit']:changeProtectedElementDataEx(reporter, "report", false, false)
					exports['antyczit']:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
					outputChatBox(getPlayerName(thePlayer) .. " zakoñczy³ zg³oszenie.", reporter, 0, 255, 255)
				end
				...
				if not isGMreport then
					...
					for key, value in ipairs(admins) do
						local adminduty = getElementData(value, "adminduty")
						if (adminduty==1) then
							outputChatBox(" [#" .. id .. "] - " .. getPlayerName(thePlayer) .. " zamyka zg³oszenie #" .. id .. ". -", value, 0, 255, 255)
						end
					end					
				else
					local admins = exports.global:getGameMasters()
					for key, value in ipairs(admins) do
						local adminduty = getElementData(value, "account:gmduty")
						if (adminduty==true) then
							outputChatBox(" [GM#" .. id .. "] - " .. getPlayerName(thePlayer) .. " zamyka zg³oszenie #" .. id .. ". -", value, 0, 255, 255)
						end
					end
				end
				updateReportCount()
...
function endReport(thePlayer, commandName)
	...
	if not (report) or not reports[report] then
		...
		reports[report] = nil
		exports['antyczit']:changeProtectedElementDataEx(thePlayer, "report", false, false)
		exports['antyczit']:changeProtectedElementDataEx(thePlayer, "reportadmin", false, false)
		
		outputChatBox("[" .. timestring .. "] Zakoñczy³eœ zg³oszenie (#" .. report .. ").", thePlayer, 255, 194, 14)
		
		if (isElement(reportHandler)) then
			outputChatBox(getPlayerName(thePlayer) .. " zakoñczy³ zg³oszenie (#" .. report .. "). Dziêkujemy za kontakt z administreacj¹.", reportHandler, 0, 255, 255)
		end
