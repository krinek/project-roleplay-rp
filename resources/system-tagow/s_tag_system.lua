...
function makeTagObject(cx, cy, cz, rot, interior, dimension)
	...
	if (tag~=9) then
		...
		exports.global:sendLocalMeAction(source, "maluje ścianę.")
		exports['antyczit']:changeProtectedElementDataEx(obj, "dbid", id, false)
		exports['antyczit']:changeProtectedElementDataEx(obj, "creator", dbid, false)
		exports['antyczit']:changeProtectedElementDataEx(obj, "type", "tag")
		outputChatBox("Zamalowałeś ścianę!", source, 255, 194, 14)
	else
		local distance = 2
		...
		if (object) then
			...
			outputChatBox("Zmazałeś brzydki tag , zarabaisz z niego 30$", source, 255, 194, 14)
...
function loadAllTags(res)
	...
	if (result) then
		...
		while run do
			...
			exports['antyczit']:changeProtectedElementDataEx(object, "dbid", id, false)
			exports['antyczit']:changeProtectedElementDataEx(object, "type", "tag")
			exports['antyczit']:changeProtectedElementDataEx(object, "creator", tonumber(row["creator"]))
			count = count + 1
			if id > highest then
				highest = id
			end
...
function setTag(thePlayer, commandName, newTag)
	if not (newTag) then
		outputChatBox("PRZYKŁAD: " .. commandName .. " [Tag # 1->8].", thePlayer, 255, 194, 14)
	elseif getElementData(thePlayer, "tag") == 9 then
		outputChatBox("nie możesz malować gdy jesteś jako czyściciel tagów", thePlayer, 255, 0, 0)
	else
		...
		if (newTag>0) and (newTag<9) then
			...
			elseif (teamName~="East Seville Saints") and (newTag==8) then -- ESS
				outputChatBox("Nie możesz użyt tego tagu.", thePlayer, 255, 0, 0)
			else
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "tag", newTag, false)
				outputChatBox("Zmieniłeś typ tagu #" .. newTag .. ".", thePlayer, 0, 255, 0)
