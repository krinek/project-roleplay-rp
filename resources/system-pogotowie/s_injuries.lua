...
function knockout()
	if playerInjuries[source] and not isTimer( playerInjuries[source]['knockout'] ) then
		outputChatBox("Zaczełeś sie wykrwawiać , szukaj pogotowia!", source, 255, 0, 0)
		...
		exports['antyczit']:changeProtectedElementDataEx(source, "injuriedanimation", true, false)
		exports.global:sendLocalMeAction(source, "stracił przytomność.")
	end
end
...
function injuries(attacker, weapon, bodypart, loss)
	...
	if ( bodypart == 3 and loss >= 15 ) or bodypart == 7 or bodypart == 8 then -- damaged either left or right leg
		...
		exports.global:sendLocalMeAction(source, " " .. ( bodypart == 7 and "lewa" or "prawa" ) .. " noga została uszkodzona.", source)
	elseif bodypart == 5 or bodypart == 6 then -- damaged either arm
		...
		exports.global:sendLocalMeAction(source, "'s " .. ( bodypart == 5 and "lewa" or "prawa" ) .. " ręka została uszkodzona.", source)
	elseif bodypart == 3 then
		exports.global:sendLocalMeAction(source, " tułów został uszkodzony.", source)
	elseif bodypart == 9 then -- headshot
		if not attacker or weapon ~= 24 or getElementData(attacker, "deaglemode") ~= 0 then
			setPedHeadless(source, true)
			exports.global:sendLocalMeAction(source, "'s głowa została odstrzelona przez " .. getPlayerName(attacker):gsub("_", " ") .. ".", source)
			killPed(source, attacker, weapon, bodypart)
			return
		end
	end
...
function stabilize()
	if playerInjuries[source] and not isPedHeadless(source) then
		if playerInjuries[source]['knockout'] then
			exports['antyczit']:changeProtectedElementDataEx(source, "injuriedanimation", false, false)
...
function examine(to)
	...
	elseif playerInjuries[source] and not isPedHeadless(source) then
		if playerInjuries[source]['knockout'] then
			outputChatBox(name.. " umiera.", to, 255, 255, 0)
		end

		if playerInjuries[source][7] and playerInjuries[source][8] then
			outputChatBox("Obie nogi u " .. name .. " są uszkodzone.", to, 255, 255, 0)
		elseif playerInjuries[source][7] then
			outputChatBox(name .. "s lewa noga jest uszkodzona.", to, 255, 255, 0)
		elseif playerInjuries[source][8] then
			outputChatBox(name .. "s prawa noga jest uszkodzona.", to, 255, 255, 0)
		end

		if playerInjuries[source][5] and playerInjuries[source][6] then
			outputChatBox("Ręce  " .. name .. "s są uszkodzone.", to, 255, 255, 0)
		elseif playerInjuries[source][5] then
			outputChatBox(name .. "s lewa ręka jest uszkodzona.", to, 255, 255, 0)
		elseif playerInjuries[source][6] then
			outputChatBox(name .. "s prawa ręka jest uszkodzona.", to, 255, 255, 0)
		end
	else
		outputChatBox(name .. " nie jest ranny.", to, 255, 255, 0)
	end
end
...
function lvesHeal(thePlayer, commandName, targetPartialNick, price)
	if not (targetPartialNick) or not (price) then -- if missing target player arg.
		outputChatBox("PRZYKŁAD: /" .. commandName .. " [Player Partial Nick / ID] [Price of Heal]", thePlayer, 255, 194, 14)
	else
		...
		if targetPlayer then -- is the player online?
			...
			if (logged==1) then
				...
				if not (factionType==4) then
					outputChatBox("Nie posiadasz podstawowych znajomości medycznych , zadzwoń po  pogotowie.", thePlayer, 255, 0, 0)
						...
						if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>3) then -- Are they standing next to each other?
							...
							if (foundkit) then
								...
								if money + bankmoney < price then
									outputChatBox("Nie masz pieniędzy na leczenie.", thePlayer, 255, 0, 0)
								else
									...
									if itemValue > 1 then
										exports['system-przedmiotow']:updateItemValue(thePlayer, slot, itemValue - 1)
									else
										exports.global:takeItem(thePlayer, 70, itemValue)
										if not exports.global:hasItem(thePlayer, 70) then
											outputChatBox("Skończyły ci sie apteczki. wpisz na nowo /duty w bazie by wziąść nowe.", thePlayer, 255, 0, 0)
										end
									end
								end
							else
								outputChatBox("Potrzebujesz apteczki by kogoś uleczyć.", thePlayer, 255, 0, 0)
							end
						end
					end
				end
...
addCommandHandler("ulecz", lvesHeal, false, false)
