...
function showPlayerMenu(targetPlayer, friend)
	...
	if not friend then
		bAddAsFriend = guiCreateButton(0.05, 0.13, 0.87, 0.1, "Dodaj jako znajomego", true, wRightClick)
		addEventHandler("onClientGUIClick", bAddAsFriend, caddFriend, false)
	else
		bAddAsFriend = guiCreateButton(0.05, 0.13, 0.87, 0.1, "Usuń znajomego", true, wRightClick)
		addEventHandler("onClientGUIClick", bAddAsFriend, cremoveFriend, false)
	end
	
	-- FRISK
	bFrisk = guiCreateButton(0.52, 0.25, 0.43, 0.1, "Przeszukaj", true, wRightClick)
	...
	if cuffed == 0 then
		bRestrain = guiCreateButton(0.05, 0.25, 0.43, 0.1, "Skuj", true, wRightClick)
		addEventHandler("onClientGUIClick", bRestrain, crestrainPlayer, false)
	else
		bRestrain = guiCreateButton(0.05, 0.25, 0.43, 0.1, "Rozkuj", true, wRightClick)
	...
	if (blindfold) and (blindfold == 1) then
		bBlindfold = guiCreateButton(0.05, 0.51, 0.87, 0.1, "Załóż opaske", true, wRightClick)
		addEventHandler("onClientGUIClick", bBlindfold, cremoveBlindfold, false)
	else
		bBlindfold = guiCreateButton(0.05, 0.51, 0.87, 0.1, "opaska", true, wRightClick)
	...
	if exports.global:hasItem(getLocalPlayer(), 70) and getElementData(player, "injuriedanimation") then
		bStabilize = guiCreateButton(0.05, y, 0.87, 0.1, "Stabilizuj", true, wRightClick)
	...
	if stretcherElement then
		...
		if stretcherPlayer and stretcherPlayer == player then
			bStabilize = guiCreateButton(0.05, y, 0.87, 0.1, "Weź z noszy", true, wRightClick)
			addEventHandler("onClientGUIClick", bStabilize, fTakeFromStretcher, false)
			y = y + 0.13
		end
		
		if not stretcherPlayer then
			bStabilize = guiCreateButton(0.05, y, 0.87, 0.1, "Połóż na noszach", true, wRightClick)
	...
	bCloseMenu = guiCreateButton(0.05, y, 0.87, 0.1, "Zamknij menu", true, wRightClick)
	addEventHandler("onClientGUIClick", bCloseMenu, hidePlayerMenu, false)
	sent = false
	
	bInformation = guiCreateButton(0.05, 0.38, 0.87, 0.1, "Informacja", true, wRightClick)
...
function cStabilize(button, state)
	if button == "left" and state == "up" then
		if (exports.global:hasItem(getLocalPlayer(), 70)) then -- Has First Aid Kit?
			...
			outputChatBox("Nie posiadasz apteczki.", 255, 0, 0)
		end
	end
end
...
function cBlindfold(button, state, x, y)
	if (button=="left") then
		if (exports.global:hasItem(getLocalPlayer(), 66)) then -- Has blindfold?
			...
			outputChatBox("Nie posiadasz opaski.", 255, 0, 0)
		end
	end
end
...
function friskShowItems(items)
	...
	triggerServerEvent("sendLocalMeAction", getLocalPlayer(), getLocalPlayer(), "przeszukuje " .. playerName .. ".")
...
fileDelete("c_player_rightclick.lua")