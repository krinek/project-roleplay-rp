...
function friskShowItems(player)
	triggerClientEvent(source, "friskShowItems", player, exports['system-przedmiotow']:getItems(player))
end
...
function restrainPlayer(player, restrainedObj)
	...
	outputChatBox("Zostałes skuty przez " .. username .. ".", player)
	outputChatBox("You are restraining " .. targetPlayerName .. ".", source)
	exports['antyczit']:changeProtectedElementDataEx(player, "restrain", 1, true)
	exports['antyczit']:changeProtectedElementDataEx(player, "restrainedObj", restrainedObj, true)
	exports['antyczit']:changeProtectedElementDataEx(player, "restrainedBy", getElementData(source, "dbid"), true)
...
function unrestrainPlayer(player, restrainedObj)
	...
	outputChatBox("zostałes odkuty przez " .. username .. ".", player)
	outputChatBox("You are unrestraining " .. targetPlayerName .. ".", source)
	
	setTimer(toggleCuffs, 200, 1, false, player)
	
	exports['antyczit']:changeProtectedElementDataEx(player, "restrain", 0)
	exports['antyczit']:changeProtectedElementDataEx(player, "restrainedBy")
	exports['antyczit']:changeProtectedElementDataEx(player, "restrainedObj")
...
function blindfoldPlayer(player)
	...
	outputChatBox("Na twoich oczach zostałą założona opaska przez " .. username .. ".", player)
	outputChatBox("Założyłeś opaske " .. targetPlayerName .. ".", source)
	
	exports.global:takeItem(source, 66) -- take their blindfold
	exports['antyczit']:changeProtectedElementDataEx(player, "blindfold", 1)
...
function removeblindfoldPlayer(player)
	...
	outputChatBox("Opaska zdjęta przez " .. username .. ".", player)
	outputChatBox("Zdjełeś opaske  " .. targetPlayerName .. "", source)
	
	exports.global:giveItem(source, 66, 1) -- give the remove the blindfold
	exports['antyczit']:changeProtectedElementDataEx(player, "blindfold")
...
function stabilizePlayer(player)
	...
	if found then
		if itemValue > 1 then
			exports['system-przedmiotow']:updateItemValue(source, slot, itemValue - 1)
		else
