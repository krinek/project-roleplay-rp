...
addEventHandler( "social:friends", localPlayer,
	function( friendsList, ownMessage, ownCountry, currentTimestamp )
		...
		-- insert yourself :o
		table.insert( online, { accountID = getElementData( localPlayer, "account:id"), name = getElementData( localPlayer, "account:username" ), message = ownMessage, country = ownCountry, player = localPlayer, editable = true })
		
		for _, data in ipairs( friendsList ) do
			if type( data[5] ) == "number" or type( data[5] ) == "string" then
				table.insert( offline, { accountID = data[1], name = data[2], message = data[3], country = data[4], lastOnline = data[5] } )
			else
				table.insert( online, { accountID = data[1], name = data[2], message = data[3], country = data[4], player = data[5] } )
			end
		end
		...
		for key, value in ipairs( data.items ) do
			...
			if inBox and justClicked then
				...
				elseif not value.editable then
					...
					friends_message_window = guiCreateWindow( screen_x / 2 - 200, screen_y / 2 - 30, 400, 60, "Ustaw wiadomosc dla znajomych", false )
					local e = guiCreateEdit( 5, 25, 290, 30, value.message, false, friends_message_window )
					local c = guiCreateButton( 300, 25, 45, 30, "Zapisz", false, friends_message_window )
					local d = guiCreateButton( 350, 25, 45, 30, "Anuluj", false, friends_message_window )
			...
			if value.player then
				text = "gra jako " .. getPlayerName( value.player ):gsub( "_", " " ) .. " (" .. getElementData( value.player, "playerid" ) .. ")."
			else
				text = ( formatTimeInterval( value.lastOnline ) .. " temu." ):gsub( "  ", " " )
			end
			...
			-- draw country flag
			dxDrawImage( w - 24, h - 16, 24, 16, "images/flags/" .. value.country:lower() .. ".png", 0, 0, 0, white, true )
