...
function showGeneralshopUI(shop_type, race, gender, discount)
	...
	if (wGeneralshop==nil) then
		...
		if not shop or #shop == 0 then
			outputChatBox("To nie jest sklep. Odejdz.")
			return
		end
		...
		local lInstruction = guiCreateLabel( 0, 20, 500, 15, "Kliknij dwa razy na przedmiot aby go kupic.", false, wGeneralshop )
		...
		for _, category in ipairs( shop ) do
			...
			local cName = guiGridListAddColumn( grid, "Nazwa", 0.25 )
			local cPrice = guiGridListAddColumn( grid, "Cena", 0.1 )
			local cDescription = guiGridListAddColumn( grid, "Opis", 0.62 )
			...
			if hasSkins then -- event handler for skin preview
				addEventHandler( "onClientGUIClick", grid, function( button, state )
					if button == "left" then
						...
						if item.itemID == 16 then
							iClothesPreview = guiCreateStaticImage( 320, 20, 100, 100, ":system-kont/img/" .. ("%03d"):format( item.itemValue or 1 ) .. ".png", false, source)
						end
					end
				end, false )
			end
		...
		bClose = guiCreateButton(200, 315, 100, 25, "Zamknij", false, wGeneralshop)
...
fileDelete("c_generalshop_system.lua")
