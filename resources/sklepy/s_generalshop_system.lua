...
function createShopKeeper(x,y,z,interior,dimension,id,shoptype,rotation, skin)
	...
	exports['antyczit']:changeProtectedElementDataEx(ped, "shopkeeper", true)
	setElementFrozen(ped, true)
	
	exports['antyczit']:changeProtectedElementDataEx(ped, "dbid", id, false)
	exports['antyczit']:changeProtectedElementDataEx(ped, "type", "shop", false)
	exports['antyczit']:changeProtectedElementDataEx(ped, "shoptype", shoptype, false)
	exports['antyczit']:changeProtectedElementDataEx(ped, "rotation", rotation, false)
end
...
function createGeneralshop(thePlayer, commandName, shoptype, skin)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		if shoptype and g_shops[shoptype] then
			...
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [shop type] [optional skin]", thePlayer, 255, 194, 14)
...
local function getDiscount( player, shoptype )
	...
	if exports.sponsorzy:hasPlayerPerk( player, 8 ) then
...
function calcSupplyCosts(thePlayer, itemID, isWeapon, supplyCost)
	if not isweapon and id ~= 68 then
		if exports.sponsorzy:hasPlayerPerk(thePlayer, 8) then
...
function getInteriorOwner( dimension )
	...
	local dbid, theEntrance, theExit, interiorType, interiorElement = exports["system-domow"]:findProperty(source)
...
addEventHandler( "shop:buy", resourceRoot, function( index )
	...
	if not exports.global:hasMoney( client, price ) then
		outputChatBox( "MAsz zbyt mało pieniędzy by zakupić " .. item.name .. ".", client, 255, 0, 0 )
		return
	end
	...
	elseif itemID == 115 or itemID == 116 then -- now here's the trick. If item.license is set, it checks for a gun license, if item.ammo is set it gives as much ammo
		if item.license and getElementData( client, "license.gun" ) ~= 1 then
			outputChatBox( "Nie posiadasz licencji na broń.", client, 255, 0, 0 )
			return
		else
			if itemID == 115 then
				local serial = "1"
			...
			elseif itemID == 116 then
				itemValue = itemValue .. ":" .. ( item.ammo or exports.bronie:getGTACap( itemValue ) or 1 ) .. ":" .. getWeaponNameFromID( itemValue )
			end
		end
	end
	...
	if dimension > 0 then -- is even in any interior
		...
		if ownerID == 0 then
			...
			suppliesToTake = item.supplies or math.ceil( 3.5 * exports['system-przedmiotow']:getItemWeight( itemID, itemValue ) )
			...
			if suppliesToTake > actualSupplies then
				outputChatBox( "Ten przedmiot obecnie jest niedostępny.", client, 255, 0, 0 )
				return
	...
	if exports.global:giveItem( client, itemID, itemValue ) then
		...
		outputChatBox( "Zakupiłeś  " .. item.name .. " za $" .. exports.global:formatMoney( price ) .. ".", client, 0, 255, 0 )
		...
		if itemID == 2 then
			...
			outputChatBox("Twój numer to #" .. itemValue .. ".", client, 255, 194, 14 )
		elseif itemID == 16 and item.fitting then -- it's a skin, so set it.
			...
			if exports['antyczit']:changeProtectedElementDataEx( client, "casualskin", itemValue, false ) then
		...
		if price > 0 and dimension > 0 then
			...
			if ownerID > 0 then -- someone even owns it
				if ownerPlayer then
					...
					exports['antyczit']:changeProtectedElementDataEx(ownerPlayer, "businessprofit", profits + price, false)
				else
		...
		outputChatBox( "Nie masz miejsca by zakupić " .. item.name .. ".", client, 255, 0, 0 )
	end
end )
...
function checkSupplies(thePlayer)
	local dbid, entrance, exit, inttype,interiorElement = exports['system-domow']:findProperty( thePlayer )
...
function handleSupplies(element, slot, event, worldItem)
	...
	if slot ~= -1 then
		item = exports['system-przedmiotow']:getItems( source )[ slot ]
	end
	...
	local dbid, entrance, exit, inttype,interiorElement = exports['system-domow']:findProperty( source )
	if (dbid==0) then
	...
	if result then
		...
		if slot == -1 and worldItem and id and isElement(worldItem) then
			...
			outputChatBox("You've added ".. amount .." supplies to this business.", source, 0, 240, 0)
			exports['system-przedmiotow']:takeItemFromSlot( source, slot )
		end
		triggerClientEvent( source, event or "finishItemMove", source )
		return
	end
