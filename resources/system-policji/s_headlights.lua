...
function vehicleBlown()
	exports['antyczit']:changeProtectedElementDataEx(source, "lspd:flashers", nil, true)
end
...
function toggleFlasherState()
	...
	if (theVehicle) then
		...
		if currentFlasherState ~= 0 then
			exports['antyczit']:changeProtectedElementDataEx(theVehicle, "lspd:flashers", nil, true)
		else
			...
			if color then
				exports['antyczit']:changeProtectedElementDataEx(theVehicle, "lspd:flashers", color, true)
			else
