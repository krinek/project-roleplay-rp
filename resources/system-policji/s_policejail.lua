...
for k, v in pairs( cells ) do
	setElementInterior( v, k <= 4 and 10 or 10 )
	setElementDimension( v, k <= 4 and 88 or 1 )
	if k <= 4 then
...
function isInArrestColshape( thePlayer )
	for k, v in pairs( cells ) do
		if isElementWithinColShape( thePlayer, v ) and getElementInterior( thePlayer ) == getElementInterior( v ) then
			return k
		end
	end
	return false
...
function destroyJailTimer ( ) -- 0001290: PD /release bug
	...
	if (isTimer(theMagicTimer)) then
		...
		exports['antyczit']:changeProtectedElementDataEx(source, "pd.jailserved", 0, false)
		exports['antyczit']:changeProtectedElementDataEx(source, "pd.jailtime", 0, false)
		exports['antyczit']:changeProtectedElementDataEx(source, "pd.jailtimer", false, false)
		exports['antyczit']:changeProtectedElementDataEx(source, "pd.jailstation", false, false)
	end
end
	...
	if (logged==1) then
		...
		if (factionType==2) and playerCol then
			if not (targetPlayerNick) or not (fine) or not (jailtime) or not (...) or (jailtime<1) or (jailtime>181) then
				outputChatBox("PRZYKŁAD: /arrest [Player Partial Nick / ID] [Fine] [Jail Time (Minutes 1->180)] [Crimes Committed]", thePlayer, 255, 194, 14)
			else
					...
					elseif targetCol ~= playerCol then
						...
						if (jailTimer) then
							...
							if not exports.global:takeMoney(targetPlayer, amount) then
								...
								exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "bankmoney", bankmoney-fineleft, false)
							end
						
							exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "pd.jailserved", 0, false)
							exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "pd.jailtime", jailtime+1, false)
							...
							if restrainedObj then
								...
								if restrainedObj == 45 then -- If handcuffs.. take the key
									...
									exports['system-przedmiotow']:deleteAll(47, dbid)
								end
							...
							exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "pd.jailstation", targetCol, false)
							...
							outputChatBox("Aresztowałeś " .. targetPlayerNick .. " na czas " .. jailtime .. " Minut.", thePlayer, 255, 0, 0)
							...
							outputChatBox("Zostałeś aresztowany przez " .. username .. " na czas " .. jailtime .. " minut.", targetPlayer, 0, 102, 255)
							outputChatBox("Popełnione przestępstwa: " .. reason .. ".", targetPlayer, 0, 102, 255)
							if (finebank == true) then
								outputChatBox("Reszta kary została odjęta z konta bankowego.", targetPlayer, 0, 102, 255)
							end
							...
							for key, value in ipairs(teamPlayers) do
								...
								outputChatBox("Popełnione Przestępstwa: " .. reason .. ".", value, 0, 102, 255)
							end
...
addCommandHandler("aresztuj", arrestPlayer)
...
function timerPDUnjailPlayer(jailedPlayer)
	if(isElement(jailedPlayer)) then
		...
		if ( timeServed ) then
			exports['antyczit']:changeProtectedElementDataEx(jailedPlayer, "pd.jailserved", tonumber(timeServed)+1, false)
			local timeLeft = timeLeft - 1
			exports['antyczit']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtime", timeLeft, false)
			...
			if (timeLeft<=0) then
				...
				if (getElementData(jailedPlayer, "pd.jailstation") <= 4) then
					...
					setElementDimension(jailedPlayer, getElementData(jailedPlayer, "pd.jailstation") <= 4 and 88 or 88)
				...
				exports['antyczit']:changeProtectedElementDataEx(jailedPlayer, "pd.jailserved", 0, false)
				exports['antyczit']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtime", 0, false)
				exports['antyczit']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtimer", false, false)
				exports['antyczit']:changeProtectedElementDataEx(jailedPlayer, "pd.jailstation", false, false)
				...
				outputChatBox("Czas aresztu sie skończył.", jailedPlayer, 0, 255, 0)
			...
			elseif (timeLeft>0) then
				...
				exports['antyczit']:changeProtectedElementDataEx(jailedPlayer, "pd.jailtimer", theTimer, false)
			end
		end
	end
end
...
function showJailtime(thePlayer)
	...
	if ajailtime then
		...
		if not (isJailed) then
			...
			outputChatBox("Masz jeszcze " .. jailtime .. " minut do odsiadki.", thePlayer, 255, 194, 14)
		end
	end
end
addCommandHandler("czaswiezienia", showJailtime)
	...
	if (logged==1) then
		...
		if factionType == 2 and isInArrestColshape(thePlayer) then
			if not (targetPlayerNick) then
				outputChatBox("PRZYKŁAD: /release [Player Partial Nick / ID]", thePlayer, 255, 194, 14)
			else
				local targetPlayer, targetPlayerNick = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					if (jailTimer) then
						exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "pd.jailtimer", nil, false)
						if isTimer(jailTimer) then
							killTimer(jailTimer)
						end
						exports['antyczit']:changeProtectedElementDataEx(targetPlayer, "pd.jailtime", 1, false)
						timerPDUnjailPlayer(targetPlayer)
