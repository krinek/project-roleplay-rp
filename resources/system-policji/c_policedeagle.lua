...
function switchMode()
	if (getPedWeapon(localPlayer)==24) and (getPedTotalAmmo(localPlayer)>0) then -- has an un-empty deagle
		...
		if mode == 0 then -- tazer mode
			...
			outputChatBox( "Zmieniłeś tryb broni na zwyczajny", 0, 255, 0 )
		elseif mode == 1 and isLSPD() then -- lethal mode
			outputChatBox( "Włączyłeś miernik prdkođi", 0, 255, 0 )
			triggerServerEvent("deaglemode", localPlayer, 2)
		elseif mode == 2 or mode == 1 then -- radar gun mode
			outputChatBox( "Zmieniłeś tryb broni na paralizator", 0, 255, 0 )
		...
		if (getElementType(target)=="vehicle") then
			if (getPedWeapon(localPlayer)==24) then
				...
				if (mode==2) then
					...
					outputChatBox(getVehicleName(target) .. " jedzie z prędkością " .. actualspeed .. " km/h.", 255, 194, 14)
				end
			end
		end
	end
