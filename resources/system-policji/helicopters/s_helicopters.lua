function unsitInHelicopter(vehicle)
	...
	if (isElement(vehicle)) then
		if not (getElementType(vehicle)=="vehicle") then
			...
			for key, value in ipairs(helicopters) do
				...
				if (players) then
					...
					if (removed) then
						exports['antyczit']:changeProtectedElementDataEx(value, "players", players, true)
					end
				end
			end
			
			exports['antyczit']:changeProtectedElementDataEx(source, "seat")
		...
		elseif (seat) and (seat>0) then
			...
			exports['antyczit']:changeProtectedElementDataEx(vehicle, "players", players, true)
			exports['antyczit']:changeProtectedElementDataEx(source, "seat")
			detachElements(source, vehicle)
			exports.global:removeAnimation(source)
		end
...
function sitInHelicopter(vehicle)
	...
	--[[for key, value in ipairs(players) do
		...
		elseif not isElementAttached ( value ) then
			exports['antyczit']:changeProtectedElementDataEx(value, "seat")
			table.remove(players, key)
	...
	if (not players) or (#players<3) then
		...
		players[seat] = source
		exports['antyczit']:changeProtectedElementDataEx(source, "seat", seat)
