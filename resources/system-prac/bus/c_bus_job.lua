...
function displayBusJob()
	...
	outputChatBox("#FF9933Udaj sie do #FF66CCblipa#FF9933 na radarze i znajdź w okolicy parking busów.", 255, 194, 15, true)
end
...
function startBusJob()
	...
	if (job == 3) then
		...
		if busMarker then
			outputChatBox("#FF9933Obecnie pracujesz.", 255, 194, 14, true)
		else
			...
			if vehicle and getVehicleController(vehicle) == getLocalPlayer() and bus[getElementModel(vehicle)] then
				...
				outputChatBox("#FF9933Podażaj  #FFCC00trasą #FF9933 zatrzymując sie na #A00101przystankach #FF9933 na drodze.", 255, 194, 14, true)
				outputChatBox("#FF9933Wypłate otrzymujesz gdy klienci wejdą do busa.", 255, 194, 14, true)
			else
				outputChatBox("#FF9933Musisz być w busie by rozpocząc prace.", 255, 194, 14, true)
			end
		end
	else
...
function endOfTheLine()
	if busNextBlip then
		...
		setVehicleLocked(vehicle, false)
		outputChatBox("#FF9933Koniec linii wpisz /startbus by pracować dalej.", 0, 255, 0, true )
	end
end
			...
			if(seat~=0)then
				...
				if driver then -- you can only pay the driver if the bus has a driver
					if not exports.global:hasMoney(getLocalPlayer(), 5)then
						...
						outputChatBox("Nie masz pieniędzy na bilet wart 5$.", 255, 0, 0)
					else
						triggerServerEvent("payBusFare", getLocalPlayer(), driver)
						outputChatBox("Zakupiłeś bilet za 5$", 0, 255, 0)
					end
				end
			elseif not busMarker and getElementData(getLocalPlayer(), "job") == 3 then
				outputChatBox("#FF9933Użyj komendy /startbus by rozpocząc prace.", 255, 0, 0, true)
			end
		end
	end
end
