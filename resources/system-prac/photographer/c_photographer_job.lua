...
local victoria = createPed(141, 356.296875, 180.5185546875, 1008.3762207031)
setPedRotation(victoria, 260)
setElementFrozen(victoria, true)
setElementDimension(victoria, 1)
			...
			if (factionType==6) then
				if (weapon == 43) then
					...
					if(pictureValue==0)then
						outputChatBox("Nikt nie będzie płacić za takie zdjęcia...", 255, 0, 0)
					else
						collectionValue = collectionValue + pictureValue
						outputChatBox("#FF9933Wartośc zdjęć: $"..exports.global:formatMoney(pictureValue), 255, 104, 91, true)
						triggerServerEvent("updateCollectionValue", localPlayer, collectionValue)
					end
					outputChatBox("#FF9933Wartośc zdjęć: $"..exports.global:formatMoney(collectionValue), 255, 104, 91, true)
				end
			end
		end
	end
...
function showValue()
	outputChatBox("#FF9933Wartośc kolekcji: $"..exports.global:formatMoney(collectionValue), 255, 104, 91, true)
end
...
function cPhotoOptions()
	...
	if not open then
		...
		sanOptionMenu = guiCreateWindow(x, y, width, height, "Jak mogę ci pomóc?", false)

		bPhotos = guiCreateButton(0.05, 0.3, 0.87, 0.25, "Sprzedaj zdjęcia", true, sanOptionMenu)
		addEventHandler("onClientGUIClick", bPhotos, startPhotos, false)

		if (getElementDimension(victoria) ~= 9902) then
			bAdvert = guiCreateButton(0.05, 0.6, 0.87, 0.25, "Ogłoś coś", true, sanOptionMenu)
	...
	function()
		...
		if getElementData(theTeam, "type") == 6 then
			if collectionValue == 0 then
				outputChatBox("Żaden z obrazów, które mają to coś warte.", 255, 0, 0, true)
			else
...
addEventHandler("updateCollectionValue", localPlayer,
	function(value)
		...
		if value > 0 then
			outputChatBox("Posiadasz zdjęcia warte $" .. exports.global:formatMoney(collectionValue) .. ".", 255, 194, 14)
		end
	end
)
