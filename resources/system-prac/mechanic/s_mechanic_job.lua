...
function serviceVehicle(veh)
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			exports.global:sendLocalMeAction(source, "serwisuje pojazd.")
...
function changeTyre( veh, wheelNumber )
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			outputChatBox("Nie posiadasz pieniędzy na ulepszenie.", source, 255, 0, 0)
		else
			...
			exports.global:sendLocalMeAction(source, "wymienia oponę.")
		end
	end
end
	...
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			if setVehiclePaintjob( veh, paintjob ) then
				...
				exports.global:sendLocalMeAction(source, "przemalowywuje pojazd.")
				exports.logs:dbLog(source, 6, {  veh }, "MODDING PAINTJOB ".. paintjob)
				exports['zapis-aut']:saveVehicleMods(veh)
			else
				outputChatBox("Ten pojazd posiada już paintjob.", source, 255, 0, 0)
			end
		end
	end
end
...
function editVehicleHeadlights( veh, color1, color2, color3 )
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			outputChatBox("Nie posiadasz funduszy na modernizacje pojazdu.", source, 255, 0, 0)
		else
			...
			if setVehicleHeadLightColor ( veh, color1, color2, color3) then
				...
				exports.global:sendLocalMeAction(source, "wymiania lampy.")
				exports['antyczit']:changeProtectedElementDataEx(veh, "headlightcolors", {color1, color2, color3}, true)
				exports['zapis-aut']:saveVehicleMods(veh)
				exports.logs:dbLog(source, 6, {  veh }, "MODDING HEADLIGHT ".. color1 .. " "..color2.." "..color3)
			else
				outputChatBox("Ten pojazd posiada już syreny.", source, 255, 0, 0)
			end
		end
	end
end
...
function changeVehicleUpgrade( veh, upgrade )
	if (veh) then
		...
		if not item and not exports.global:hasMoney( source, mechcost ) then
			outputChatBox("Nie posiadasz funduszy by zamontować " .. name .. " do tego pojazdu.", source, 255, 0, 0)
		else
			for i = 0, 16 do
			...
			if addVehicleUpgrade( veh, upgrade ) then
				...
				outputChatBox("Błąd przy montowaniu częsci.", source, 255, 0, 0)
			end
		end
	end
end
	...
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			if setVehicleColor( veh, color1[1], color1[2], color1[3], color2[1], color2[2], color2[3],  color3[1], color3[2], color3[3], color4[1], color4[2], color4[3]) then
				...
				exports.global:sendLocalMeAction(source, "przemalowywuje autko.")
				exports['zapis-aut']:saveVehicleMods(veh)
	...
	if veh and stat then
		if stat == 1 then
			...
			if leader == 1 then
				...
				if not exports.global:takeMoney(source, mechcost) then
					outputChatBox("Nie posiadasz funduszy by zakupić przyciemnienie szyb.", source, 255, 0, 0)
				else
		...
		elseif stat == 2 then
			...
			if not exports.global:takeMoney(source, mechcost) then
				...
				if query then
					...
					exports['antyczit']:changeProtectedElementDataEx(veh, "tinted", false, true)
