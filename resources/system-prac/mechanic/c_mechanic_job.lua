...
function displayMechanicJob()
	outputChatBox("#FF9933Uzyj #FF0000menu pod prawym przyciskiem myszy#FF9933 by rozpocząć prace", 255, 194, 15, true)
end
	...
	if (job==5) or (faction==30)then
		if not vehicle then
			...
			if not (wMechanic) then
				-- Create the window
				wMechanic = guiCreateWindow(X, Y, Width, Height, "Opcje mechanika.", false )
				
				local y = 0.05
				-- Body work
				--bMechanicOne = guiCreateButton( 0.05, y, 0.9, 0.1, "Naprawa Karoserii - $50", true, wMechanic )
				--addEventHandler( "onClientGUIClick", bMechanicOne, bodyworkTrigger, false)
				--y = y + 0.1
				
				-- Service
				bMechanicTwo = guiCreateButton( 0.05, y, 0.9, 0.1, "Załatać dziury - $110", true, wMechanic )
				addEventHandler( "onClientGUIClick", bMechanicTwo, serviceTrigger, false)
				y = y + 0.1
				
				-- Tyre Change
				if not noTyres[getVehicleType(vehicle)] then
					bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Wymiana opon - $10", true, wMechanic )
				...
				if (playerDimension > 0) then
					...
					if faction == 30 or ( tonumber( getElementData(vehicle, "job") or 0 ) == 0 and ( getElementData(vehicle, "faction") == -1 or getElementData(vehicle, "faction") == faction ) ) then
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Przemaluj auto - $100", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Wymień lampy - $4,000", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction == 30 then
						bMechanicSeven = guiCreateButton( 0.05, y, 0.9, 0.1, "Usuń NOS", true, wMechanic )
					...
					if faction == 30 then
						if not getElementData(vehicle, "tinted") then
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Zainstaluj odcień szyb - $5,500", true, wMechanic )
							addEventHandler( "onClientGUIClick", bMechanicEight, addTintWindow, false)
							y = y + 0.1
						else
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Usuń odcień szyb - $2,000", true, wMechanic )
				...
				-- Close
				bMechanicClose = guiCreateButton( 0.05, 0.85, 0.9, 0.1, "Zamknij", true, wMechanic )
...
function tyreWindow()
	...
	if not (wTyre) then
		-- Create the window
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Wybierz oponę do zmiany.", false )
		
		if getVehicleType(currentVehicle) ~= "Bike" then
			-- Front left
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.45, 0.35, "Przednia lewa", true, wTyre )
			...
			-- Back left
			bTyreTwo = guiCreateButton( 0.05, 0.5, 0.45, 0.35, "Tylna Lewa", true, wTyre )
			...
			-- front right
			bTyreThree = guiCreateButton( 0.5, 0.1, 0.45, 0.35, "Przednia prawa", true, wTyre )
			...
			-- back right
			bTyreFour = guiCreateButton( 0.5, 0.5, 0.45, 0.35, "Tylna prawa", true, wTyre )
			...
			-- Front
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.9, 0.35, "Cały przód", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreOne, function(button, state)
				if(button == "left" and state == "up") then
			...
			-- back right
			bTyreThree = guiCreateButton( 0.05, 0.5, 0.9, 0.35, "Cały tył", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreThree, function(button, state)
				if(button == "left" and state == "up") then
		...
		-- Close
		bTyreClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Zamknij", true, wTyre )
...
function paintWindow()
	...
	if not (wPaint) then
		wPaint = guiCreateWindow(windowX, windowY, windowWidth, windowHeight, "Malowanie pojazdu", false )
		bPaintColor1 = guiCreateButton( 0.05, 0.1, 0.45, 0.2, "Wybierz kolor 1", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor1,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 1 )
			end
		end, false)	 
		bPaintColor2 = guiCreateButton( 0.55, 0.1, 0.45, 0.2, "Wybierz kolor 2", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor2,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 2 )
			end
		end, false)	
		bPaintColor3 = guiCreateButton( 0.05, 0.4, 0.45, 0.2, "Wybierz kolor 3", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor3,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 3 )
			end
		end, false)	
		bPaintColor4 = guiCreateButton( 0.55, 0.4, 0.45, 0.2, "Wybierz kolor 4", true, wPaint )
		...
		bPaintSubmit = guiCreateButton( 0.05, 0.75, 0.90, 0.1, "Zapisz", true, wPaint )
		...
		bPaintClose = guiCreateButton( 0.05, 0.85, 0.90, 0.1, "Zamknij", true, wPaint )
	...
	if not (wPaint) then
		...
		-- Create the window
		wPaint = guiCreateWindow(X, Y, Width, Height, "Wybierz kolory do malowania pojazdu.", false )
...
function replaceLightsWindow()
	...
	if not (wPaint) then
		...
		-- Close
		bPaintClose = guiCreateButton( 0.35, 0.8, 0.3, 0.2, "Zamknij", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintClose,  function(button, state)
			if(button == "left" and state == "up") then
