...
local function showFishMarker(msg)
	if not (fishingBlip) then -- If the /sellfish marker isnt already being shown...
		...
		if msg == 1 then
			outputChatBox("#FF9933Gdy zakończysz łowienie ryb udaj sie do  #FF66CCrybnego marketu #FF9933.", 255, 104, 91, true)
			outputChatBox("#FF9933((/polow aby zobaczyć, jak dużo masz złapane rybek. Aby sprzedać swój połów wpisz /sprzedajryby w #FF66CCmarkecie#FF9933.))", 255, 104, 91, true)
		elseif msg == 2 then
			outputChatBox("#FF9933Obecnie masz już " .. totalCatch .. " lbs rybek.", 255, 104, 91, true)
		end
	end
end
		...
		if not (getElementType(element)=="vehicle") then
			...
			if not (getVehicleType(element)=="Boat") then
				...
				if ((y < 3000) and ( y > -3000)) and ((x > -3000) and (x < 3000)) then -- Are they out at sea.
					outputChatBox("Musisz być daleko na wodzie by łowić.", 255, 0, 0)
				else
					if (catchTimer) then -- Are they already fishing?
						outputChatBox("Zarzuciłeś już linę , zaczekaj na ryby.", 255, 0, 0)
					else
						totalCatch = oldcatch
						if (totalCatch >= 2000) then
							outputChatBox("#FF9933W łodzi nie ma miejsca na rybki. #FF66CCSprzedaj#FF9933 rybki w sklepie i kontynuuj.", 255, 104, 91, true)
							showFishMarker()
						else
...
function fishOnLine()
	...
	if ((y < 3000) and ( y > -3000)) and ((x > -3000) and (x < 3000)) then
		...
		outputChatBox("Masz coś! ((klikaj [ oraz ] by zwijać wędke ))", 0, 255, 0)
...
function reelItIn()
	...
	if (count>=100) then
		...
		outputChatBox("Obecnie masz "..totalCatch.."lbs rybek.", 255, 194, 14)
...
addCommandHandler("polow", currentCatch, false, false)
...
addCommandHandler("sprzedajryby", unloadCatch, false, false)

fileDelete("c_fishing_job.lua")
