...
function startCleaningMission(thePlayer)
	if (isElement(thePlayer)) then
		...
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "fuel", 100)
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "owner", -2, false)
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "faction", -1, false)
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "dbid", 999999, false)
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "oldx", 1503.5179443359, false)
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "oldy", 369.0305175781, false)
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "oldz", 10.8203125, false)
		exports['antyczit']:changeProtectedElementDataEx(vehicle, "engine", 1, false)
		removePedFromVehicle(thePlayer)
		warpPedIntoVehicle(thePlayer, vehicle)
		
		exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.marker", "1")
		exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.vehicle", vehicle)
		...
		if tonumber(int) == 1 then
			...
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.checkmarkers", "25")
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.t", "1")
		elseif tonumber(int) == 2 then
			x1 = CheckpointStyle_2[1].x_point
			y1 = CheckpointStyle_2[1].y_point
			z1 = CheckpointStyle_2[1].z_point
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.checkmarkers", "17")
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.t", "2")
		elseif tonumber(int) == 3 then
			x1 = CheckpointStyle_3[1].x_point
			y1 = CheckpointStyle_3[1].y_point
			z1 = CheckpointStyle_3[1].z_point
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.checkmarkers", "17")
			exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.t", "3")
		end																				--- <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		...
		exports['antyczit']:changeProtectedElementDataEx(colsphere, "attatched", marker2)
	...
	if ( isPedInVehicle ( thePlayer ) ) then
		...
		if tonumber(vehid) == 574 then
			...
			if tonumber(max_number) == tonumber(m_number) then
				...
				exports['antyczit']:changeProtectedElementDataEx(colsphere, "attatched", marker3)
				
				addEventHandler("onColShapeHit", colsphere, FinalCheckpoints)
			else
				local newnumber = m_number+1
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.marker", newnumber)
...
function FinalCheckpoints( thePlayer )
	if ( isPedInVehicle ( thePlayer ) ) then
		...
		if tonumber(vehid) == 574 then
			...
			exports['antyczit']:changeProtectedElementDataEx(colsphere, "attatched", marker4)
	...
	if (isElement(thePlayer)) then
		if ( isPedInVehicle ( thePlayer ) ) then
			...
			if tonumber(vehid) == 574 then
				...
				exports['antyczit']:changeProtectedElementDataEx(vehicle, "fuel")
				exports['antyczit']:changeProtectedElementDataEx(vehicle, "dbid")
				
				if (isElement(vehicle)) then
					destroyElement(vehicle)
					vehicle = nil
					exports['antyczit']:changeProtectedElementDataEx(thePlayer, "cleaner.vehicle")
...
function quit()
	...
	exports['antyczit']:changeProtectedElementDataEx(source, "cleaner.vehicle")
end
