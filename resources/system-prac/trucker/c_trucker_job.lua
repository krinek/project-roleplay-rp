...
function displayTruckerJob(notext)
	if (jobstate==0) then
		...
		if not notext then
			outputChatBox("#FF9933Udaj sie do #CCCCCCblipa#FF9933 oraz wejdź do pojazdu by rozpocząć prace", 255, 194, 15, true)
		end
	end
end
...
function startTruckerJob(routeid)
	if (jobstate==1) then
		...
		if vehicle and getVehicleController(vehicle) == localPlayer and truck[getElementModel(vehicle)] then
			outputChatBox("#FF9933Jedź do #FFFF00blipów#FF9933 by dostarczyc pierwszy towar.", 255, 194, 15, true)
			outputChatBox("#FF9933Pamiętaj również o #FFFF00zasadach ruchu drogowego#FF9933.", 255, 194, 15, true)
			outputChatBox("#FF9933Jeśli twój pojazd jest #FFFF00zniszczony#FF9933, towar bedzie niższej wartości.", 255, 194, 15, true)
			outputChatBox("#FF9933Nie zniszcz wozu, #FFFF00inaczej towar sie zepsuje#FF9933!", 255, 194, 15, true)
			destroyElement(blip)
...
function waitAtDelivery(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer and truck[getElementModel(vehicle)] then
		if getElementHealth(vehicle) < 350 then
			...
			outputChatBox("#FF9933Zaczekaj chwilę , towar z twojego pojazdu dostawczego jest wypakowywany.", 255, 0, 0, true )
...
function nextDeliveryCheckpoint()
	...
	if jobstate == 2 or jobstate == 3 then
		...
		if vehicle and getVehicleController(vehicle) == localPlayer and truck[getElementModel(vehicle)] then
			...
			outputChatBox("#FF9933Musisz byc w vanie by zakonczyc prace.", 255, 0, 0, true ) -- Wrong car type.
		end
	end
end
...
function endDelivery(thePlayer)
	if thePlayer == localPlayer then
		...
		if not vehicle or getVehicleController(vehicle) ~= localPlayer or not (truck[id]) then
			...
			local health = getElementHealth(vehicle)
			if health <= 700 then
				outputChatBox("Twoj Van jest zniszczony , musisz go naprawic.", 255, 194, 15)
			else
