...
function saveVehicle(source)
	...
	if isElement(source) and getElementType(source) == "vehicle" and dbid >= 0 then
		...
		--if (owner~=-1) then
			...
			local maxVelocity = getElementData(source, "maxVelocity") or 0
			local engineAcceleration = getElementData(source, "engineAcceleration") or 0
			local steeringLock = getElementData(source, "steeringLock") or 0
			local neony = getElementData(source, "neonyava") or 0
