				...
				for k, v in ipairs( value["spawnpoints"] ) do
					...
					if v["vehicle"] and isElement( v["vehicle"] ) and data then
						...
						if getElementModel(v.vehicle) == model or setElementModel(v.vehicle, model) then
							...
							exports['antyczit']:changeProtectedElementDataEx(v["vehicle"], "carshop:cost", data[2], false)
							exports['antyczit']:changeProtectedElementDataEx(v["vehicle"], "carshop:taxcost", 2*(vehicleTaxes[getVehicleModelFromName(data[1])-399] or 25), false)
							setElementFrozen(v["vehicle"], false)
							setVehicleVariant(v.vehicle, exports['system-aut']:getRandomVariant(getElementModel(v.vehicle)))
						...
						if canPopulate2 then
							...
							if not vehicle then
								...
								setVehicleVariant(vehicle, exports['system-aut']:getRandomVariant(getElementModel(vehicle)))
								v["vehicle"] = vehicle

								local x = v[1] - ( ( math.cos ( math.rad (  v[6] ) ) ) * 1.5 )
								local y = v[2] - ( ( math.sin ( math.rad (  v[6] ) ) ) * 1.5 )
								local tempPickup = createPickup(x, y, v[3], 3, 1239)
								exports['antyczit']:changeProtectedElementDataEx(tempPickup, "carshop:parentCar", v["vehicle"], false)
								exports['antyczit']:changeProtectedElementDataEx(v["vehicle"], "carshop:cost", data[2], false)
								exports['antyczit']:changeProtectedElementDataEx(v["vehicle"], "carshop", true, false)
								exports['antyczit']:changeProtectedElementDataEx(v["vehicle"], "carshop:childPickup", tempPickup, false)
								exports['antyczit']:changeProtectedElementDataEx(v["vehicle"], "carshop:taxcost", 2*(vehicleTaxes[getVehicleModelFromName(data[1])-399] or 25), false)
								
								--[[ Second hand: future use
								exports['antyczit']:changeProtectedElementDataEx(v["vehicle"], "carshop:secondhand", data[3], false])
								if (data[3]) then
			...
			if (isCarshopVehicle) then
				...
				elseif (paymentMethod == "bank") then
					...
					if money < 0 or costCar == 0 then
						...
						exports['antyczit']:changeProtectedElementDataEx(client, "bankmoney", money, false)
				...
				if (insertid) then
					...
					call( getResourceFromName( "system-przedmiotow" ), "deleteAll", 3, insertid )
					...
					exports['system-aut']:reloadVehicle(insertid)
					outputChatBox("Gratulacje zakupiłeś pojazd!", client)
					outputChatBox("Upewnij sie że wpisałeś komędę /zaparkuj w miejscu gdzie chcesz go zapisać,", client)
					outputChatBox("w przeciwnym razie twój pojazd zostanie usunięty bez odwrotu.", client)
				end
			end
		else
			outputChatBox("Potrzebujesz prawo jazdy by zakupić pojazd.", client, 255, 0, 0)
			return
		end
	end
end
