...
function loadOneTrafficCam(id)
	...
	if (row) then
		...
		exports['antyczit']:changeProtectedElementDataEx(colTube, "speedcam", true, false)
		exports['antyczit']:changeProtectedElementDataEx(colTube, "speedcam:dbid", id, false)
		exports['antyczit']:changeProtectedElementDataEx(colTube, "speedcam:maxspeed", maxspeed, false)
		exports['antyczit']:changeProtectedElementDataEx(colTube, "speedcam:enabled", enabled, false)
...
function createTrafficCam(thePlayer, commandName, maxSpeed)
	if (exports.global:isPlayerLeadAdmin(thePlayer)) then
		if not (maxSpeed) then
			outputChatBox("PRZYKŁAD: /" .. commandName .. " [trigger speed in KPH]", thePlayer, 255, 194, 14)
		else
			if (tonumber(maxSpeed) > 35) then
				...
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new", true, false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new:x", x, false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new:y", y, false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new:z", z, false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new:dimension", dimension, false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new:interior", interior, false)
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new:maxspeed", tonumber(maxSpeed), false)
	...
	if (exports.global:isPlayerLeadAdmin(thePlayer)) then
		...
		if (checkPoint) then
			...
			if (radius > 99) then
				...
				exports['antyczit']:changeProtectedElementDataEx(thePlayer, "speedcam:new", false, false)
...
function toggleTrafficCam(theColshape)
	...
	if (isSpeedcam) then
		...
		local dbid = getElementData(theColshape, "speedcam:dbid")
		exports['antyczit']:changeProtectedElementDataEx(theColshape, "speedcam:enabled",  not currentStatus, false)
